...
function achievement_invite_update($data)
{
    ...
    if (mysqli_num_rows($res) > 0) {
        ...
        while ($arr = mysqli_fetch_assoc($res)) {
            ...
            if ($invited >= 1 && $lvl == 0) {
                $msg = sqlesc('Congratulations, you have just earned the [b]Inviter Level 1[/b] achievement. :) [img]' . $INSTALLER09['baseurl'] . '/images/achievements/invite1.png[/img]');
            ...
            if ($invited >= 2 && $lvl == 1) {
                $msg = sqlesc('Congratulations, you have just earned the [b]Inviter Level 2[/b] achievement. :) [img]' . $INSTALLER09['baseurl'] . '/images/achievements/invite2.png[/img]');
                $msgs_buffer[] = '(0,' . $arr['userid'] . ',' . TIME_NOW . ', ' . sqlesc($msg) . ', ' . sqlesc($subject) . ')';
            ...
            if ($invited >= 3 && $lvl == 2) {
                $msg = sqlesc('Congratulations, you have just earned the [b]Inviter Level 3[/b] achievement. :) [img]' . $INSTALLER09['baseurl'] . '/images/achievements/invite3.png[/img]');
                $msgs_buffer[] = '(0,' . $arr['userid'] . ',' . TIME_NOW . ', ' . sqlesc($msg) . ', ' . sqlesc($subject) . ')';
            ...
            if ($invited >= 5 && $lvl == 3) {
                $msg = sqlesc('Congratulations, you have just earned the [b]Inviter Level 4[/b] achievement. :) [img]' . $INSTALLER09['baseurl'] . '/images/achievements/invite4.png[/img]');
                $msgs_buffer[] = '(0,' . $arr['userid'] . ',' . TIME_NOW . ', ' . sqlesc($msg) . ', ' . sqlesc($subject) . ')';
            ...
            if ($invited >= 10 && $lvl == 4) {
                $msg = sqlesc('Congratulations, you have just earned the [b]Inviter Level 5[/b] achievement. :) [img]' . $INSTALLER09['baseurl'] . '/images/achievements/invite5.png[/img]');
                $msgs_buffer[] = '(0,' . $arr['userid'] . ',' . TIME_NOW . ', ' . sqlesc($msg) . ', ' . sqlesc($subject) . ')';
