...
function achievement_shouts_update($data)
{
    ...
    if (mysqli_num_rows($res) > 0) {
        ...
        while ($arr = mysqli_fetch_assoc($res)) {
            ...
            if ($shouts >= 1 && $lvl == 0) {
                $msg = sqlesc('Congratulations, you have just earned the [b]AJAX Chat Spammer Level 1[/b] achievement. :) [img]' . $INSTALLER09['baseurl'] . '/images/achievements/spam1.png[/img]');
            ...
            if ($shouts >= 25 && $lvl == 1) {
                $msg = sqlesc('Congratulations, you have just earned the [b]AJAX Chat Spammer Level 2[/b] achievement. :) [img]' . $INSTALLER09['baseurl'] . '/images/achievements/spam2.png[/img]');
                $msgs_buffer[] = '(0,' . $arr['userid'] . ',' . TIME_NOW . ', ' . sqlesc($msg) . ', ' . sqlesc($subject) . ')';
            ...
            if ($shouts >= 50 && $lvl == 2) {
                $msg = sqlesc('Congratulations, you have just earned the [b]AJAX Chat Spammer Level 3[/b] achievement. :) [img]' . $INSTALLER09['baseurl'] . '/images/achievements/spam3.png[/img]');
                $msgs_buffer[] = '(0,' . $arr['userid'] . ',' . TIME_NOW . ', ' . sqlesc($msg) . ', ' . sqlesc($subject) . ')';
            ...
            if ($shouts >= 75 && $lvl == 3) {
                $msg = sqlesc('Congratulations, you have just earned the [b]AJAX Chat Spammer Level 4[/b] achievement. :) [img]' . $INSTALLER09['baseurl'] . '/images/achievements/spam4.png[/img]');
                $msgs_buffer[] = '(0,' . $arr['userid'] . ',' . TIME_NOW . ', ' . sqlesc($msg) . ', ' . sqlesc($subject) . ')';
            ...
            if ($shouts >= 100 && $lvl == 4) {
                $msg = sqlesc('Congratulations, you have just earned the [b]AJAX Chat Spammer Level 5[/b] achievement. :) [img]' . $INSTALLER09['baseurl'] . '/images/achievements/spam5.png[/img]');
                $msgs_buffer[] = '(0,' . $arr['userid'] . ',' . TIME_NOW . ', ' . sqlesc($msg) . ', ' . sqlesc($subject) . ')';
