...
function hitrun_xbt_update($data)
{
    ...
    if ($site_config['hnr_online'] == 1) {
        ...
        while ($arr_fuckers = mysqli_fetch_assoc($res_fuckers)) {
            if ($arr_fuckers['poop'] > $site_config['cainallowed'] && $arr_fuckers['downloadpos'] == 1) {
                //=== set them to no DLs
                ...
                $mc1->commit_transaction($site_config['expires']['u_stats_xbt']);
                $mc1->begin_transaction('user_stats_xbt' . $arr_fuckers['uid']);
                $mc1->update_row(false, [
                    'modcomment' => $modcomment,
                ]);
                $mc1->commit_transaction($site_config['expires']['user_stats_xbt']);
        ...
        while ($arr_good_boy = mysqli_fetch_assoc($res_good_boy)) {
            ...
            if ($arr_count[0] < $site_config['cainallowed']) {
                //=== set them to yes DLs
