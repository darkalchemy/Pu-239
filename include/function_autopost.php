...
function auto_post($subject = 'Error - Subject Missing', $body = 'Error - No Body') // Function to use the special system message forum
{
    ...
    if (mysqli_num_rows($res) == 1) { // Topic already exists in the system forum.
        ...
        sql_query("INSERT INTO topics (user_id, forum_id, topic_name) VALUES({$INSTALLER09['chatBotID']}, {$INSTALLER09['staff']['forumid']}, $subject)") or sqlerr(__FILE__, __LINE__);
    ...
    $added = TIME_NOW;
    sql_query('INSERT INTO posts (topic_id, user_id, added, body) ' . 'VALUES(' . sqlesc($topicid) . ", {$INSTALLER09['chatBotID']}, $added, " . sqlesc($body) . ')') or sqlerr(__FILE__, __LINE__);
