...
if ($count === 0) {
    ...
    while ($arr_unread = mysqli_fetch_assoc($res_unread)) {
        ...
        if ($arr_post_read[0] < $arr_unread['last_post']) {
            ...
            $icon = (empty($arr_unread['icon']) ? '<img src="' . $site_config['paths']['images_baseurl'] . 'forums/topic_normal.gif" class="icon tooltipper" alt="' . $lang['fe_topic'] . '" title="' . $lang['fe_topic'] . '">' : '<img src="' . $site_config['paths']['images_baseurl'] . 'smilies/' . htmlsafechars($arr_unread['icon']) . '.gif" class="icon tooltipper" alt="' . $lang['fe_unread'] . '" title="' . $lang['fe_unread'] . '">');
