...
$HTMLOUT .= $mini_menu . '<h1>' . $lang['vup_unread_post_since_visit'] . '</h1>';
        ...
        if ($arr_post_read[0] < $arr_unread['last_post']) {
            ...
            if ($arr_unread['tan'] === 'yes') {
                if ($CURUSER['class'] < UC_STAFF && $arr_unread['user_id'] != $CURUSER['id']) {
                    ...
                    $thread_starter = ($arr_unread['username'] !== '' ? '<i>' . $lang['fe_anonymous'] . '</i> [' . format_username($arr_unread['id']) . ']' : '' . $lang['fe_lost'] . ' [' . (int) $arr_unread['id'] . ']') . '<br>' . get_date($first_unread_poster_arr[0], '');
                }
            } else {
                $thread_starter = ($arr_unread['username'] !== '' ? format_username($arr_unread['id']) : '' . $lang['fe_lost'] . ' [' . (int) $arr_unread['id'] . ']') . '<br>' . get_date($first_unread_poster_arr[0], '');
            }
            ...
            $icon = (empty($arr_unread['icon']) ? '<img src="' . $site_config['pic_baseurl'] . 'forums/topic_normal.gif" alt="' . $lang['fe_topic'] . '" title="' . $lang['fe_topic'] . '" />' : '<img src="' . $site_config['pic_baseurl'] . 'smilies/' . htmlsafechars($arr_unread['icon']) . '.gif" alt="' . $lang['fe_unread'] . '" title="' . $lang['fe_unread'] . '" />');
