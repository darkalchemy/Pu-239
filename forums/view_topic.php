...
if ($arr['poll_id'] > 0) {
	...
	<tr>
	<td class="forum_head_dark" colspan="2" align="left"><img src="' . $site_config['pic_base_url'] . 'forums/poll.gif" alt="" /><span style="font-weight: bold;">Poll
	...
	<td class="forum_head_dark" colspan="3" align="right">' . ($CURUSER['class'] < UC_STAFF ? '' : '<a href="' . $site_config['baseurl'] . '/forums.php?action=poll&amp;action_2=poll_edit&amp;topic_id=' . $topic_id . '" class="altlink"><img src="' . $site_config['pic_base_url'] . 'forums/modify.gif" alt="" width="20px" /> ' . $lang['fe_edit'] . '</a>  
	...
	<tr>
	<td class="three" width="5px" align="center"><img src="' . $site_config['pic_base_url'] . 'forums/poll_question.png" alt="" width="25px" /></td>
    ...
    for ($i = 0; $i < $number_of_options; ++$i) {
		...
		<tr>
		<td style="padding: 0px; background-image: url(' . $site_config['pic_base_url'] . 'forums/vote_img_bg.gif); background-repeat: repeat-x">
	   <img src="' . $site_config['pic_base_url'] . 'forums/vote_img.gif" width="' . $math . '%" height="8" alt="' . $math_text . '" title="' . $math_text . '"  /></td>
		...
		<td class="' . $class . '" align="center">' . (in_array($i, $members_votes) ? '<img src="' . $site_config['pic_base_url'] . 'forums/check.gif" width="20px" alt=" " /> <span style="font-weight: bold;">' . $lang['fe_your_vote'] . '!</span>' : '') . '</td></tr>';
    }
...
$subscription_on_off = (isset($_GET['s']) ? ($_GET['s'] == 1 ? '<br><div style="font-weight: bold;">' . $lang['fe_sub_to_topic'] . ' <img src="' . $site_config['pic_base_url'] . 'forums/subscribe.gif" alt="' . $lang['fe_subscribed'] . '" title="' . $lang['fe_subscribed'] . '"  width="25" /></div>' : '<br><div style="font-weight: bold;">' . $lang['fe_unsub_to_topic'] . ' <img src="' . $site_config['pic_base_url'] . 'forums/unsubscribe.gif" alt="' . $lang['fe_unsubscribe'] . '" title="' . $lang['fe_unsubscribe'] . '" width="25" /></div>') : '');
...
//=== reply button
$locked_or_reply_button = ($locked === 'yes' ? '<span style="font-weight: bold; font-size: x-small;"><img src="' . $site_config['pic_base_url'] . 'forums/thread_locked.gif" alt="' . $lang['fe_thread_locked'] . '" title="' . $lang['fe_thread_locked'] . '" width="22" />' . $lang['fe_this_topic_is_locked'] . ', you may not post in this thread.</span>' : ($CURUSER['forum_post'] == 'no' ? '<span style="font-weight: bold; font-size: x-small;">Your posting rights have been removed. You may not post.</span>' : '<a href="' . $site_config['baseurl'] . '/forums.php?action=post_reply&amp;topic_id=' . $topic_id . '" class="btn">Add Reply</a>'));
/*
...
$location_bar = '<a name="top"></a>' . $status_image . ' <a class="altlink" href="index.php">' . $site_config['site_name'] . '</a>  <img src="' . $site_config['pic_base_url'] . 'forums/arrow_next.gif" alt="&#9658;" title="&#9658;" /> 
			...
			<span style="text-align: center;">' . $mini_menu . (($topic_owner == $CURUSER['id'] && $arr['poll_id'] == 0 || $CURUSER['class'] >= UC_STAFF && $arr['poll_id'] == 0) ? '  |<a href="' . $site_config['baseurl'] . '/forums.php?action=poll&amp;action_2=poll_add&amp;topic_id=' . $topic_id . '" class="altlink">&#160;' . $lang['fe_add_poll'] . '</a>' : '') . '</span><br><br>';
$HTMLOUT .= ($upload_errors_size > 0 ? ($upload_errors_size === 1 ? '<div style="text-align: center;">One file was not uploaded. The maximum file size allowed is. ' . mksize($max_file_size) . '.</div>' : '<div style="text-align: center;">' . $upload_errors_size . ' file were not uploaded. The maximum file size allowed is. ' . mksize($max_file_size) . '.</div>') : '') . ($upload_errors_type > 0 ? ($upload_errors_type === 1 ? '<div style="text-align: center;">One file was not uploaded. The accepted formats are zip and rar.</div>' : '<div style="text-align: center;">' . $upload_errors_type . ' files were not uploaded. The accepted formats are zip and rar.</div>') : '') . $location_bar . $topic_poll . '<br>' . $subscription_on_off . '<br>
      ...
      <form action="' . $site_config['baseurl'] . '/forums.php?action=staff_actions" method="post" name="checkme" onsubmit="return SetChecked(this,\'post_to_mess_with\')" enctype="multipart/form-data">') . (isset($_GET['count']) ? '<div style="text-align: center;">' . intval($_GET['count']) . ' PMs Sent</div>' : '') . '


		<!--<table border="0" cellspacing="5" cellpadding="10" width="100%">-->
		<table border="0" cellspacing="0" cellpadding="4" width="100%">
		' . $the_top_and_bottom . '
		<tr><td class="forum_head_dark" align="left" width="100"> <img src="' . $site_config['pic_base_url'] . 'forums/topic_normal.gif" alt="' . $lang['fe_topic'] . '" title="' . $lang['fe_topic'] . '" />&#160;&#160;' . $lang['fe_author'] . '</td>
...
while ($arr = mysqli_fetch_assoc($res)) {
    ...
    if (mysqli_num_rows($attachments_res) > 0) {
        ...
        while ($attachments_arr = mysqli_fetch_assoc($attachments_res)) {
            $attachments .= '<span style="white-space:nowrap;">' . ($attachments_arr['extension'] === 'zip' ? ' <img src="' . $site_config['pic_base_url'] . 'forums/zip.gif" alt="' . $lang['fe_zip'] . '" title="' . $lang['fe_zip'] . '" width="18" style="vertical-align: middle;" /> ' : ' <img src="' . $site_config['pic_base_url'] . 'forums/rar.gif" alt="' . $lang['fe_rar'] . '" title="' . $lang['fe_rar'] . '" width="18" /> ') . ' 
      ...
      <span class="tip">' . ($arr['anonymous'] == 'yes' ? '<i>' . $lang['fe_anonymous'] . '</i>' : htmlsafechars($arr['username'])) . ' ' . $moodname . ' !</span></a>&#160;</span>
	     ...
	     ' . ($CURUSER['class'] == UC_MAX && $arr['staff_lock'] == 1 ? '<a href="' . $site_config['baseurl'] . '/forums.php?action=staff_lock&amp;mode=unlock&amp;post_id=' . $post_id . '&amp;topic_id=' . $topic_id . '"><img src="' . $site_config['pic_base_url'] . 'key.gif" alt="' . $lang['fe_un_lock'] . '" title="' . $lang['fe_un_lock'] . '" /> ' . $lang['fe_unlock_post'] . '</a>&#160;' : '') . '
		  ...
		  <a href="' . $site_config['baseurl'] . '/forums.php?action=view_topic&amp;topic_id=' . $topic_id . '&amp;page=' . $page . '#bottom"><img src="' . $site_config['pic_base_url'] . 'forums/down.gif" alt="' . $lang['fe_bottom'] . '" title="' . $lang['fe_bottom'] . '" /></a> 
			...
			<tr>
         <td class="' . $class_alt . '" align="center" valign="top">' . ($arr['anonymous'] == 'yes' ? '<img style="max-width:' . $width . 'px;" src="' . $site_config['pic_base_url'] . 'anonymous_1.jpg" alt="avatar" />' : avatar_stuff($arr)) . '<br>
			...
			<span style="color: green;"><img src="' . $site_config['pic_base_url'] . 'up.png" alt="' . $lang['vt_uploaded'] . '" title="' . $lang['vt_uploaded'] . '" /> ' . mksize($arr['uploaded']) . '</span>&#160;&#160;  
			' . ($site_config['ratio_free'] ? '' : '<span style="color: red;"><img src="' . $site_config['pic_base_url'] . 'dl.png" alt="' . $lang['vt_downloaded'] . '" title="' . $lang['vt_downloaded'] . '" /> ' . mksize($arr['downloaded']) . '</span>') . '&#160;&#160;') . (($arr['paranoia'] >= 2 && $CURUSER['class'] < UC_STAFF) ? '' : '' . $lang['vt_ratio'] . ': ' . member_ratio($arr['uploaded'], $site_config['ratio_free'] ? '0' : $arr['downloaded']) . '&#160;&#160;
    ...
    <td class="forum_head_dark" colspan="4" align="center">' . $lang['fe_staff_tools'] . '</td>
      ...
      <table border="0" cellspacing="2" cellpadding="2" width="100%" align="center">
		  <tr>
			...
			<td class="two" align="center" valign="middle" width="18"><img src="' . $site_config['pic_base_url'] . 'forums/delete.gif" alt="' . $lang['fe_delete'] . '" title="' . $lang['fe_delete'] . '" /></td>
			<td class="two" align="left" valign="middle">
      ...
      <table border="0" cellspacing="2" cellpadding="2" width="100%" align="center">
		  <tr>
			...
			<td class="two" align="center" valign="middle" width="18"><img src="' . $site_config['pic_base_url'] . 'forums/send_pm.png" alt="' . $lang['vt_send_pm'] . '" title="' . $lang['vt_send_pm'] . '" width="18" /></td>
