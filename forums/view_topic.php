...
$attachments = $members_votes = $status = $topic_poll = $stafflocked = $child = $parent_forum_name = $math_image = $math_text = $staff_tools = $staff_link = $now_viewing = '';
...
switch ($status) {
    ...
    case 'recycled':
        $status = 'recycled';
        $status_image = '<img src="' . $site_config['pic_base_url'] . 'forums/recycle_bin.gif" alt="' . $lang['fe_recycled'] . '" title="' . $lang['fe_this_thread_is_currently'] . ' ' . $lang['fe_in_the_recycle_bin'] . '" />';
        break;

    case 'deleted':
        $status = 'deleted';
        $status_image = '<img src="' . $site_config['pic_base_url'] . 'forums/delete_icon.gif" alt="' . $lang['fe_deleted'] . '" title="' . $lang['fe_this_thread_is_currently'] . ' ' . $lang['fe_deleted'] . '" />';
        break;
}
//=== topics stuff
...
if ($arr['poll_id'] > 0) {
    ...
    $topic_poll .= (($voted === 1 || $poll_open === 0) ? '<br><br>' : '<form action="' . $site_config['baseurl'] . '/forums.php?action=poll" method="post" name="poll">
	<fieldset class="poll_select">
	<input type="hidden" name="topic_id" value="' . $topic_id . '" />
	<input type="hidden" name="action_2" value="poll_vote" />') . '
	<table class="table table-bordered table-striped">
	<tr>
	<td class="forum_head_dark" colspan="2"><img src="' . $site_config['pic_base_url'] . 'forums/poll.gif" alt="" /><span>Poll
	...
	<td class="forum_head_dark" colspan="3">' . ($CURUSER['class'] < UC_STAFF ? '' : '<a href="' . $site_config['baseurl'] . '/forums.php?action=poll&amp;action_2=poll_edit&amp;topic_id=' . $topic_id . '" class="altlink"><img src="' . $site_config['pic_base_url'] . 'forums/modify.gif" alt="" width="20px" /> ' . $lang['fe_edit'] . '</a>  
	<a href="' . $site_config['baseurl'] . '/forums.php?action=poll&amp;action_2=poll_reset&amp;topic_id=' . $topic_id . '" class="altlink"><img src="' . $site_config['pic_base_url'] . 'forums/stop_watch.png" alt=" " width="20px" /> ' . $lang['fe_reset'] . '</a> 
	' . (($arr_poll['poll_ends'] > TIME_NOW || $arr_poll['poll_closed'] === 'no') ? '<a href="' . $site_config['baseurl'] . '/forums.php?action=poll&amp;action_2=poll_close&amp;topic_id=' . $topic_id . '" class="altlink"><img src="' . $site_config['pic_base_url'] . 'forums/clock.png" alt="" width="20px" /> close</a>' : '<a href="' . $site_config['baseurl'] . '/forums.php?action=poll&amp;action_2=poll_open&amp;topic_id=' . $topic_id . '" class="altlink"><img src="' . $site_config['pic_base_url'] . 'forums/clock.png" alt="" width="20px" /> ' . $lang['fe_start'] . '</a>') . '
	<a href="' . $site_config['baseurl'] . '/forums.php?action=poll&amp;action_2=poll_delete&amp;topic_id=' . $topic_id . '" class="altlink"><img src="' . $site_config['pic_base_url'] . 'forums/delete.gif" alt="" width="20px" /> ' . $lang['fe_delete'] . '</a>') . '</td>
	</tr>
	<tr>
	<td><img src="' . $site_config['pic_base_url'] . 'forums/poll_question.png" alt="" width="25px" /></td>
	<td colspan="4"><br>' . format_comment($arr_poll['question']) . '<br><br></td>
	</tr>
	<tr>
	<td colspan="5">' . (($voted === 1 || $poll_open === 0) ? '' : '<p>you may select up to <span>' . $multi_options . ' </span>option' . ($multi_options == 1 ? '' : 's') . '.</p>') . '</td>
	</tr>';
    ...
    for ($i = 0; $i < $number_of_options; ++$i) {
        ...
        if ($voted === 1) {
            ...
            $math_image = '<table class="table table-bordered table-striped">
		<tr>
		<td>
	   <img src="' . $site_config['pic_base_url'] . 'forums/vote_img.gif" width="' . $math . '%" height="8" alt="' . $math_text . '" title="' . $math_text . '"  /></td>
	   </tr></table>';
        }
        $topic_poll .= '<tr><td>' . (($voted === 1 || $poll_open === 0) ? '<span>' . ($i + 1) . '.</span>' : ($multi_options == 1 ? '<input type="radio" name="vote" value="' . $i . '" />' : '<input type="checkbox" name="vote[]" id="vote[]" value="' . $i . '" />')) . '</td>
		<td>' . format_comment($poll_options[$i]) . '</td>
		<td>' . $math_image . '</td>
		<td><span>' . $math_text . '</span></td>
		<td>' . (in_array($i, $members_votes) ? '<img src="' . $site_config['pic_base_url'] . 'forums/check.gif" width="20px" alt=" " /> <span>' . $lang['fe_your_vote'] . '!</span>' : '') . '</td></tr>';
    }
    $topic_poll .= (($change_vote === 1 && $voted === 1) ? '<tr><td colspan="5">
			<a href="' . $site_config['baseurl'] . '/forums.php?action=poll&amp;action_2=reset_vote&amp;topic_id=' . $topic_id . '" class="altlink"><img src="' . $site_config['pic_base_url'] . 'forums/stop_watch.png" alt="" width="20px" /> ' . $lang['fe_reset_your_vote'] . '!</a> 
			</td></tr>' : '') . ($voted === 1 ? '
	     <tr>
			...
			<div id="voters">' . $who_voted . '</div>') . '</td>
	</tr>
</table><br>' : ($poll_open === 0 ? '' : '<tr>
			<td>' . ($multi_options == 1 ? '<input type="radio" name="vote" value="666" />' : '<input type="checkbox" name="vote[]" id="vote[]" value="666" />') . '</td>
			<td colspan="4"><span>' . $lang['fe_i_just_want_to_see_the_results'] . '!</span></td>
		</tr>') . (($voted === 1 || $poll_open === 0) ? '</table><br>' : '<tr><td colspan="5">
...
$forum_desc = ($arr['topic_desc'] !== '' ? '<span>' . htmlsafechars($arr['topic_desc'], ENT_QUOTES) . '</span><br><br>' : '');
...
//=== staff options
if ($CURUSER['class'] >= UC_STAFF) {
    $staff_link = '<a class="altlink"  title="' . $lang['fe_staff_tools'] . '" id="tool_open">' . $lang['fe_staff_tools'] . '</a>';
}
//=== rate topic \o/
...
$subscriptions = ($row_subscriptions[0] > 0 ? ' <a class="altlink" href="' . $site_config['baseurl'] . '/forums.php?action=delete_subscription&amp;topic_id=' . $topic_id . '"> 
		<img src="' . $site_config['pic_base_url'] . 'forums/unsubscribe.gif" alt="+" title="+" width="12" /> ' . $lang['fe_unsubscribe_from_this_topic'] . '</a>' : '<a class="altlink" href="' . $site_config['baseurl'] . '/forums.php?action=add_subscription&amp;forum_id=' . $forum_id . '&amp;topic_id=' . $topic_id . '">
		<img src="' . $site_config['pic_base_url'] . 'forums/subscribe.gif" alt="+" title="+" width="12" /> ' . $lang['fe_subscribe_to_this_topic'] . '</a>');
//=== who is here
...
$subscription_on_off = (isset($_GET['s']) ? ($_GET['s'] == 1 ? '<br><div>' . $lang['fe_sub_to_topic'] . ' <img src="' . $site_config['pic_base_url'] . 'forums/subscribe.gif" alt="' . $lang['fe_subscribed'] . '" title="' . $lang['fe_subscribed'] . '"  width="25" /></div>' : '<br><div>' . $lang['fe_unsub_to_topic'] . ' <img src="' . $site_config['pic_base_url'] . 'forums/unsubscribe.gif" alt="' . $lang['fe_unsubscribe'] . '" title="' . $lang['fe_unsubscribe'] . '" width="25" /></div>') : '');
...
//=== reply button
$locked_or_reply_button = ($locked === 'yes' ? '<span><img src="' . $site_config['pic_base_url'] . 'forums/thread_locked.gif" alt="' . $lang['fe_thread_locked'] . '" title="' . $lang['fe_thread_locked'] . '" width="22" />' . $lang['fe_this_topic_is_locked'] . ', you may not post in this thread.</span>' : ($CURUSER['forum_post'] == 'no' ? '<span>Your posting rights have been removed. You may not post.</span>' : '<a href="' . $site_config['baseurl'] . '/forums.php?action=post_reply&amp;topic_id=' . $topic_id . '" class="btn">Add Reply</a>'));
/*
    $quick_reply ="<table class='table table-bordered table-striped'>
      <tr>
        <td>
        <b>Quick Reply</b>
...
$the_top_and_bottom = '<tr><td width="33%">&#160;&#160;' . $subscriptions . '</td>
		<td width="33%">' . (($count > $perpage) ? $menu : '') . '</td>
		<td>' . ($may_post ? $locked_or_reply_button : '<span>
		You are not permitted to post in this thread.</span>') . '</td></tr>';
$location_bar = '<a name="top"></a>' . $status_image . ' <a class="altlink" href="index.php">' . $site_config['site_name'] . '</a>  <img src="' . $site_config['pic_base_url'] . 'forums/arrow_next.gif" alt="&#9658;" title="&#9658;" /> 
			<a class="altlink" href="' . $site_config['baseurl'] . '/forums.php">' . $lang['fe_forums'] . '</a> ' . $parent_forum_name . ' 
			<img src="' . $site_config['pic_base_url'] . 'forums/arrow_next.gif" alt="&#9658;" title="&#9658;" /> 
			<a class="altlink" href="' . $site_config['baseurl'] . '/forums.php?action=view_forum&amp;forum_id=' . $forum_id . '">' . $forum_name . $child . '</a>
			<img src="' . $site_config['pic_base_url'] . 'forums/arrow_next.gif" alt="&#9658;" title="&#9658;" /> 
			<a class="altlink" href="' . $site_config['baseurl'] . '/forums.php?action=view_topic&amp;topic_id=' . $topic_id . '">' . $topic_name . '</a> ' . $status_image . '<br>' . $forum_desc . '
			<span>' . $mini_menu . (($topic_owner == $CURUSER['id'] && $arr['poll_id'] == 0 || $CURUSER['class'] >= UC_STAFF && $arr['poll_id'] == 0) ? '  |<a href="' . $site_config['baseurl'] . '/forums.php?action=poll&amp;action_2=poll_add&amp;topic_id=' . $topic_id . '" class="altlink">&#160;' . $lang['fe_add_poll'] . '</a>' : '') . '</span><br><br>';
$HTMLOUT .= ($upload_errors_size > 0 ? ($upload_errors_size === 1 ? '<div>One file was not uploaded. The maximum file size allowed is. ' . mksize($max_file_size) . '.</div>' : '<div>' . $upload_errors_size . ' file were not uploaded. The maximum file size allowed is. ' . mksize($max_file_size) . '.</div>') : '') . ($upload_errors_type > 0 ? ($upload_errors_type === 1 ? '<div>One file was not uploaded. The accepted formats are zip and rar.</div>' : '<div>' . $upload_errors_type . ' files were not uploaded. The accepted formats are zip and rar.</div>') : '') . $location_bar . $topic_poll . '<br>' . $subscription_on_off . '<br>
		' . ($CURUSER['class'] < UC_STAFF ? '' : '

      <form action="' . $site_config['baseurl'] . '/forums.php?action=staff_actions" method="post" name="checkme" onsubmit="return SetChecked(this,\'post_to_mess_with\')" enctype="multipart/form-data">') . (isset($_GET['count']) ? '<div>' . intval($_GET['count']) . ' PMs Sent</div>' : '') . '


		<table class="table table-bordered table-striped">
		' . $the_top_and_bottom . '
		<tr><td class="forum_head_dark" width="100"> <img src="' . $site_config['pic_base_url'] . 'forums/topic_normal.gif" alt="' . $lang['fe_topic'] . '" title="' . $lang['fe_topic'] . '" />&#160;&#160;' . $lang['fe_author'] . '</td>
		<td class="forum_head_dark" colspan="2">&#160;&#160;' . $lang['fe_topic'] . ': ' . $topic_name . '  [ ' . $lang['fe_read'] . ' ' . $views . ' ' . $lang['fe_times'] . ' ] </td></tr>
		<tr><td colspan="3">' . $lang['fe_topic_rating'] . ': ' . (getRate($topic_id, 'topic')) . '</td></tr>
      <tr><td colspan="3">' . $topic_users . '</td></tr>';
...
while ($arr = mysqli_fetch_assoc($res)) {
    ...
    $post_icon = ($arr['icon'] !== '' ? '<img src="' . $site_config['pic_base_url'] . 'smilies/' . htmlsafechars($arr['icon']) . '.gif" alt="icon" title="icon" /> ' : '<img src="' . $site_config['pic_base_url'] . 'forums/topic_normal.gif" alt="icon" title="icon" /> ');
    $post_title = ($arr['post_title'] !== '' ? ' <span>' . htmlsafechars($arr['post_title'], ENT_QUOTES) . '</span>' : '');
    $stafflocked = ( /*$CURUSER['class'] == UC_SYSOP && */
    $arr['staff_lock'] == 1 ? "<img src='{$site_config['pic_base_url']}locked.gif' border='0' alt='" . $lang['fe_post_locked'] . "' title='" . $lang['fe_post_locked'] . "' />" : '');
    ...
    if (mysqli_num_rows($attachments_res) > 0) {
        $attachments = '<table class="table table-bordered table-striped"><tr><td><span>' . $lang['fe_attachments'] . ':</span><hr>';
        while ($attachments_arr = mysqli_fetch_assoc($attachments_res)) {
            $attachments .= '<span>' . ($attachments_arr['extension'] === 'zip' ? ' <img src="' . $site_config['pic_base_url'] . 'forums/zip.gif" alt="' . $lang['fe_zip'] . '" title="' . $lang['fe_zip'] . '" width="18" /> ' : ' <img src="' . $site_config['pic_base_url'] . 'forums/rar.gif" alt="' . $lang['fe_rar'] . '" title="' . $lang['fe_rar'] . '" width="18" /> ') . ' 
    ...
    $height = 100;
    //=== signature stuff
    $signature = (($CURUSER['opt1'] & user_options::SIGNATURES) ? '' : ($arr['signature'] == '' ? '' : ($arr['anonymous'] == 'yes' || $arr['perms'] & bt_options::PERMS_STEALTH ? '<table class="table table-bordered table-striped"><tr><td><hr><img style="max-width:' . $width . 'px;max-height:' . $height . 'px;" src="' . $site_config['pic_base_url'] . 'anonymous_2.jpg" alt="Signature" /></td></tr></table>' : '<table class="table table-bordered table-striped"><tr><td><hr>' . format_comment($arr['signature']) . '</td></tr></table>')));
    //=== post status
    ...
    $width = 100;
    $HTMLOUT .= '<tr><td colspan="3"><table class="table table-bordered table-striped"><tr><td class="forum_head" width="100">
			<span><a name="' . $post_id . '"></a>
			...
			<img src="' . $site_config['pic_base_url'] . 'forums/link.gif" alt="' . $lang['fe_direct_link_to_this_post'] . '" title="' . $lang['fe_direct_link_to_this_post'] . '" width="12px" /></a>
			<span>' . ($arr['anonymous'] == 'yes' ? '<i>' . $lang['fe_anonymous'] . '</i>' : '' . htmlsafechars($arr['username']) . '') . '&#160;</span>
			<!-- Mood -->
         <span class="tool"><a href="javascript:;" onclick="PopUp(\'usermood.php\',\'Mood\',530,500,1,1);"><img src="' . $site_config['pic_base_url'] . 'smilies/' . $moodpic . '" alt="' . $moodname . '" border="0" />
      <span class="tip">' . ($arr['anonymous'] == 'yes' ? '<i>' . $lang['fe_anonymous'] . '</i>' : htmlsafechars($arr['username'])) . ' ' . $moodname . ' !</span></a>&#160;</span>
			' . (($arr['paranoia'] >= 2 && $CURUSER['class'] < UC_STAFF) ? '<img src="' . $site_config['pic_base_url'] . 'smilies/tinfoilhat.gif" alt="' . $lang['fe_i_wear_a_tinfoil_hat'] . '!" title="' . $lang['fe_i_wear_a_tinfoil_hat'] . '!" />' : get_user_ratio_image($arr['uploaded'], ($site_config['ratio_free'] ? '0' : $arr['downloaded']))) . '</span>
			</td>
			<td class="forum_head"><span>' . $post_icon . $post_title . '&#160;&#160;&#160;&#160; ' . $lang['fe_posted_on'] . ': ' . get_date($arr['added'], '') . ' [' . get_date($arr['added'], '', 0, 1) . ']</span></td>
			<td class="forum_head"><span>
			<a class="altlink" href="' . $site_config['baseurl'] . '/forums.php?action=post_reply&amp;topic_id=' . $topic_id . '&amp;quote_post=' . $post_id . '&amp;key=' . $arr['added'] . '"><img src="' . $site_config['pic_base_url'] . 'forums/quote.gif" alt="' . $lang['fe_quote'] . '" title="' . $lang['fe_quote'] . '" /> ' . $lang['fe_quote'] . '</a>
			' . (($CURUSER['class'] >= UC_STAFF || $CURUSER['id'] == $arr['id']) ? ' <a class="altlink" href="' . $site_config['baseurl'] . '/forums.php?action=edit_post&amp;post_id=' . $post_id . '&amp;topic_id=' . $topic_id . '&amp;page=' . $page . '"><img src="' . $site_config['pic_base_url'] . 'forums/modify.gif" alt="' . $lang['fe_modify'] . '" title="' . $lang['fe_modify'] . '" /> ' . $lang['fe_modify'] . '</a> 
			 <a class="altlink" href="' . $site_config['baseurl'] . '/forums.php?action=delete_post&amp;post_id=' . $post_id . '&amp;topic_id=' . $topic_id . '"><img src="' . $site_config['pic_base_url'] . 'forums/delete.gif" alt="' . $lang['fe_delete'] . '" title="' . $lang['fe_delete'] . '" /> ' . $lang['fe_remove'] . '</a>' : '') . '
			 <!--<a class="altlink" href="' . $site_config['baseurl'] . '/forums.php?action=report_post&amp;topic_id=' . $topic_id . '&amp;post_id=' . $post_id . '"><img src="' . $site_config['pic_base_url'] . 'forums/report.gif" alt="' . $lang['fe_report'] . '" title="' . $lang['fe_report'] . '" width="22" /> ' . $lang['fe_report'] . '</a>-->
			 <a href="' . $site_config['baseurl'] . '/report.php?type=Post&amp;id=' . $post_id . '&amp;id_2=' . $topic_id . '"><img src="' . $site_config['pic_base_url'] . 'forums/report.gif" alt="' . $lang['fe_report'] . '" title="' . $lang['fe_report'] . '" width="22" /> ' . $lang['fe_report'] . '</a>
	     ' . ($CURUSER['class'] == UC_MAX && $arr['staff_lock'] == 1 ? '<a href="' . $site_config['baseurl'] . '/forums.php?action=staff_lock&amp;mode=unlock&amp;post_id=' . $post_id . '&amp;topic_id=' . $topic_id . '"><img src="' . $site_config['pic_base_url'] . 'key.gif" alt="' . $lang['fe_un_lock'] . '" title="' . $lang['fe_un_lock'] . '" /> ' . $lang['fe_unlock_post'] . '</a>&#160;' : '') . '
			 ' . ($CURUSER['class'] == UC_MAX && $arr['staff_lock'] == 0 ? '<a href="' . $site_config['baseurl'] . '/forums.php?action=staff_lock&amp;mode=lock&amp;post_id=' . $post_id . '&amp;topic_id=' . $topic_id . '"><img src="' . $site_config['pic_base_url'] . 'key.gif" alt="' . $lang['fe_lock'] . '" title="' . $lang['fe_lock'] . '" /> ' . $lang['fe_lock_post'] . '</a>' : '') . $stafflocked . '
			<a href="' . $site_config['baseurl'] . '/forums.php?action=view_topic&amp;topic_id=' . $topic_id . '&amp;page=' . $page . '#top"><img src="' . $site_config['pic_base_url'] . 'forums/up.gif" alt="' . $lang['fe_top'] . '" title="' . $lang['fe_top'] . '" /></a> 
		  <a href="' . $site_config['baseurl'] . '/forums.php?action=view_topic&amp;topic_id=' . $topic_id . '&amp;page=' . $page . '#bottom"><img src="' . $site_config['pic_base_url'] . 'forums/down.gif" alt="' . $lang['fe_bottom'] . '" title="' . $lang['fe_bottom'] . '" /></a> 
			</span></td>
			</tr>	
			<tr>
         <td>' . ($arr['anonymous'] == 'yes' ? '<img style="max-width:' . $width . 'px;" src="' . $site_config['pic_base_url'] . 'anonymous_1.jpg" alt="avatar" />' : avatar_stuff($arr)) . '<br>
			' . ($arr['anonymous'] == 'yes' ? '<i>' . $lang['fe_anonymous'] . '</i>' : print_user_stuff($arr)) . ($arr['anonymous'] == 'yes' || $arr['title'] == '' ? '' : '<br><span>[' . htmlsafechars($arr['title']) . ']</span>') . '<br>
			<span>' . ($arr['anonymous'] == 'yes' ? '' : get_user_class_name($arr['class'])) . '</span><br>
			' . ($arr['last_access'] > (TIME_NOW - 300) && $arr['perms'] < bt_options::PERMS_STEALTH ? ' <img src="' . $site_config['pic_base_url'] . 'online.gif" alt="Online" title="Online" border="0" /> Online' : ' <img src="' . $site_config['pic_base_url'] . 'forums/offline.gif" border="0" alt="' . $lang['fe_offline'] . '" title="' . $lang['fe_offline'] . '" /> ' . $lang['fe_offline'] . '') . '<br>
			' . $lang['fe_karma'] . ': ' . number_format($arr['seedbonus']) . '<br><br>' . $member_reputation . '<br>' . ($arr['google_talk'] !== '' ? ' <a href="http://talkgadget.google.com/talkgadget/popout?member=' . htmlsafechars($arr['google_talk']) . '" title="' . $lang['fe_click_for_google_talk_gadget'] . '"  target="_blank"><img src="' . $site_config['pic_base_url'] . 'forums/google_talk.gif" alt="' . $lang['fe_google_talk'] . '" /></a> ' : '') . ($arr['icq'] !== '' ? ' <a href="http://people.icq.com/people/&amp;uin=' . htmlsafechars($arr['icq']) . '" title="' . $lang['fe_click_to_open_icq_page'] . '" target="_blank"><img src="' . $site_config['pic_base_url'] . 'forums/icq.gif" alt="icq" /></a> ' : '') . ($arr['msn'] !== '' ? ' <a href="http://members.msn.com/' . htmlsafechars($arr['msn']) . '" target="_blank" title="' . $lang['fe_click_to_see_msn_details'] . '"><img src="' . $site_config['pic_base_url'] . 'forums/msn.gif" alt="msn" /></a> ' : '') . ($arr['aim'] !== '' ? ' <a href="http://aim.search.aol.com/aol/search?s_it=searchbox.webhome&amp;q=' . htmlsafechars($arr['aim']) . '" target="_blank" title="' . $lang['fe_click_to_search_on_aim'] . '"><img src="' . $site_config['pic_base_url'] . 'forums/aim.gif" alt="AIM" /></a> ' : '') . ($arr['yahoo'] !== '' ? ' <a href="http://webmessenger.yahoo.com/?im=' . htmlsafechars($arr['yahoo']) . '" target="_blank" title="' . $lang['fe_click_to_open_yahoo'] . '"><img src="' . $site_config['pic_base_url'] . 'forums/yahoo.gif" alt="yahoo" /></a> ' : '') . '<br><br>' . ($arr['website'] !== '' ? ' <a href="' . htmlsafechars($arr['website']) . '" target="_blank" title="' . $lang['fe_click_to_go_to_website'] . '"><img src="' . $site_config['pic_base_url'] . 'forums/website.gif" alt="website" /></a> ' : '') . ($arr['show_email'] == 'yes' ? ' <a href="mailto:' . htmlsafechars($arr['email']) . '"  title="' . $lang['fe_click_to_email'] . '" target="_blank"><img src="' . $site_config['pic_base_url'] . 'email.gif" alt="email" width="25" /> </a>' : '') . '<br><br>
			' . ($CURUSER['class'] >= UC_STAFF ? '   
			<ul class="makeMenu">
			...
			<tr><td></td><td colspan="2">' . $signature . '</td></tr>
			<tr><td></td><td colspan="2">' . $attachments . '</td></tr>
			<tr><td colspan="3">' . (($arr['paranoia'] >= 1 && $CURUSER['class'] < UC_STAFF) ? '' : '
			<span><img src="' . $site_config['pic_base_url'] . 'up.png" alt="' . $lang['vt_uploaded'] . '" title="' . $lang['vt_uploaded'] . '" /> ' . mksize($arr['uploaded']) . '</span>&#160;&#160;  
			' . ($site_config['ratio_free'] ? '' : '<span><img src="' . $site_config['pic_base_url'] . 'dl.png" alt="' . $lang['vt_downloaded'] . '" title="' . $lang['vt_downloaded'] . '" /> ' . mksize($arr['downloaded']) . '</span>') . '&#160;&#160;') . (($arr['paranoia'] >= 2 && $CURUSER['class'] < UC_STAFF) ? '' : '' . $lang['vt_ratio'] . ': ' . member_ratio($arr['uploaded'], $site_config['ratio_free'] ? '0' : $arr['downloaded']) . '&#160;&#160;
			' . ($arr['hit_and_run_total'] == 0 ? '<img src="' . $site_config['pic_base_url'] . 'forums/no_hit_and_runs2.gif" width="22" alt="' . ($arr['anonymous'] == 'yes' ? '' . $lang['fe_anonymous'] . '' : htmlsafechars($arr['username'])) . ' ' . $lang['vt_has_never_hit'] . ' &amp; ran!" title="' . ($arr['anonymous'] == 'yes' ? '' . $lang['fe_anonymous'] . '' : htmlsafechars($arr['username'])) . ' ' . $lang['vt_has_never_hit'] . ' &amp; ran!" />' : '') . '
			&#160;&#160;&#160;&#160;') . '
			<a class="altlink" href="pm_system.php?action=send_message&amp;receiver=' . $arr['id'] . '&amp;returnto=' . urlencode($_SERVER['REQUEST_URI']) . '"><img src="' . $site_config['pic_base_url'] . 'forums/send_pm.png" alt="' . $lang['vt_send_pm'] . '" title="' . $lang['vt_send_pm'] . '" width="18" /> ' . $lang['vt_send_message'] . '</a></td></tr></table></td></tr>';
    $attachments = '';
} //=== end while loop
...
$HTMLOUT .= $the_top_and_bottom . '</table>
    ...
    <br>' . ($CURUSER['class'] >= UC_STAFF ? '<img src="' . $site_config['pic_base_url'] . 'forums/tools.png" alt="' . $lang['vt_tools'] . '" title="' . $lang['vt_tools'] . '" width="22" /> ' . $staff_link . ' <img src="' . $site_config['pic_base_url'] . 'forums/tools.png" alt="' . $lang['vt_tools'] . '" title="' . $lang['vt_tools'] . '" width="22" /><br><br>
	 <div id="tools">
    <br>
    <table class="table table-bordered table-striped">
	 <tr>
    <td class="forum_head_dark" colspan="4">' . $lang['fe_staff_tools'] . '</td>
      ...
      <table class="table table-bordered table-striped">
		  <tr>
			<td width="18">
			<img src="' . $site_config['pic_base_url'] . 'forums/recycle_bin.gif" alt="' . $lang['vt_recycle'] . '" title="' . $lang['vt_recycle'] . '" width="22" /></td>
			<td>
			...
			<td width="18"><img src="' . $site_config['pic_base_url'] . 'forums/delete.gif" alt="' . $lang['fe_delete'] . '" title="' . $lang['fe_delete'] . '" /></td>
			<td>
			...
			<td width="18">
			<img src="' . $site_config['pic_base_url'] . 'forums/merge.gif" alt="' . $lang['vt_merge'] . '" title="' . $lang['vt_merge'] . '" /></td>
			<td>
      ...
      <table class="table table-bordered table-striped">
		  <tr>
			<td width="18">
			<img src="' . $site_config['pic_base_url'] . 'forums/split.gif" alt="' . $lang['vt_split'] . '" title="' . $lang['vt_split'] . '" width="18" /></td>
			<td>
			...
			<td width="18"><img src="' . $site_config['pic_base_url'] . 'forums/send_pm.png" alt="' . $lang['vt_send_pm'] . '" title="' . $lang['vt_send_pm'] . '" width="18" /></td>
			<td>
			<a class="altlink"  title="' . $lang['vt_send_pm_select_mem'] . ' - click" id="pm_open">' . $lang['vt_send_pm'] . ' </a><br>[click]
			</td>
		  </tr>
	    </table>
      <div id="pm"><br>
      ...
      <tr>
			<td width="28">
			<img src="' . $site_config['pic_base_url'] . 'forums/pinned.gif" alt="' . $lang['fe_pinned'] . '" title="' . $lang['fe_pinned'] . '" /></td>
			<td>
      ...
      <tr>
			<td width="28">
			<img src="' . $site_config['pic_base_url'] . 'forums/thread_locked.gif" alt="' . $lang['fe_locked'] . '" title="' . $lang['fe_locked'] . '" width="22" /></td>
			<td>
	...
	<!-- needed to add later RS.	 -->				
         <img src="' . $site_config['pic_base_url'] . 'forums/move.gif" alt="' . $lang['vt_move'] . '" title="' . $lang['vt_move'] . '" width="22" /></td>
			<td>
...
<form action="' . $site_config['baseurl'] . '/forums.php?action=staff_actions" method="post">
      ...
      <tr>
			<td width="28">
			<img src="' . $site_config['pic_base_url'] . 'forums/modify.gif" alt="' . $lang['fe_modify'] . '" title="' . $lang['fe_modify'] . '" /></td>
			<td>
      ...
      <tr>
			<td width="28">
			<img src="' . $site_config['pic_base_url'] . 'forums/delete.gif" alt="' . $lang['fe_delete'] . '" title="' . $lang['fe_delete'] . '" /></td>
			<td>
