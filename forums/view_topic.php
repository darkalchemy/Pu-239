...
$subscriptions = $subscribed ? "
        <a href='{$site_config['baseurl']}/forums.php?action=delete_subscription&amp;topic_id={$topic_id}'>{$lang['fe_unsubscribe_from_this_topic']}</a>" : "
        <a href='{$site_config['baseurl']}/forums.php?action=add_subscription&amp;forum_id={$forum_id}&amp;topic_id={$topic_id}'>{$lang['fe_subscribe_to_this_topic']}</a>";
...
$like_button = "
                <div class='level-right margin10'>
...
$locked_or_reply_button = $locked === 'yes' ? "
    <span class='tooltipper' title='{$lang['fe_this_topic_is_locked']}, you may not post in this thread.'>{$lang['fe_this_topic_is_locked']}" : ($CURUSER['forum_post'] === 'no' ? "
    <span class='tooltipper' title='Your posting rights have been removed. You may not post.'>Disabled</span>" : "
    <a href='{$site_config['baseurl']}/forums.php?action=post_reply&amp;topic_id={$topic_id}'>Add Reply</a>");
            ...
            <td colspan="3">
                <div class="columns level-center">
                    <div class="column is-one-quarter level">
                        ...
                        <img src="' . $image . '" data-src="' . $site_config['pic_baseurl'] . 'smilies/tinfoilhat.gif" alt="' . $lang['fe_i_wear_a_tinfoil_hat'] . '!" title="' . $lang['fe_i_wear_a_tinfoil_hat'] . '!" class="tooltipper emoticon lazy">' : get_user_ratio_image($usersdata['uploaded'], (RATIO_FREE ? '0' : $usersdata['downloaded']))) . '
                    </div>
                    ...
                    <div class="column">
                        ...
                        ' . ($CURUSER['class'] >= $site_config['staff_allowed']['lock_topics'] && $arr['staff_lock'] == 1 ? '<a href="' . $site_config['baseurl'] . '/forums.php?action=staff_lock&amp;mode=unlock&amp;post_id=' . $post_id . '&amp;topic_id=' . $topic_id . '" title="' . $lang['fe_un_lock'] . '" class="tooltipper"><i class="icon-key icon"></i>' . $lang['fe_unlock_post'] . '</a>' : '') . '
                        ' . ($CURUSER['class'] >= $site_config['staff_allowed']['lock_topics'] && $arr['staff_lock'] == 0 ? '<a href="' . $site_config['baseurl'] . '/forums.php?action=staff_lock&amp;mode=lock&amp;post_id=' . $post_id . '&amp;topic_id=' . $topic_id . '" title="' . $lang['fe_lock'] . '" class="tooltipper"><i class="icon-key icon"></i>' . $lang['fe_lock_post'] . '</a>' : '') . $stafflocked . '
         ...
         <td class="has-text-centered w-15 mw-150">' . get_avatar($usersdata) . '<br>
			...
			<tr>
			    <td colspan="3">' . (($usersdata['paranoia'] >= 1 && $CURUSER['class'] < UC_STAFF) ? '' : '
                    ...
                    ' . (RATIO_FREE ? '' : '<span style="color: red;"><img src="' . $image . '" data-src="' . $site_config['pic_baseurl'] . 'dl.png" alt="' . $lang['vt_downloaded'] . '" title="' . $lang['vt_downloaded'] . '" class="tooltipper emoticon lazy"> ' . mksize($usersdata['downloaded']) . '</span>') . '') . (($usersdata['paranoia'] >= 2 && $CURUSER['class'] < UC_STAFF) ? '' : '' . $lang['vt_ratio'] . ': ' . member_ratio($usersdata['uploaded'], RATIO_FREE ? '0' : $usersdata['downloaded']) . '
