...
switch ($staff_action) {
    ...
    case 'set_pinned':
        ...
        sql_query('UPDATE topics SET sticky = \'' . ('yes' === $_POST['pinned'] ? 'yes' : 'no') . '\' WHERE id = ' . sqlesc($topic_id));
        clr_forums_cache($topic_id);
        header('Location: forums.php?action=view_topic&topic_id=' . $topic_id);
        die();
        break;
    ...
    case 'set_locked':
        ...
        sql_query('UPDATE topics SET locked = \'' . ('yes' === $_POST['locked'] ? 'yes' : 'no') . '\' WHERE id = ' . sqlesc($topic_id));
        clr_forums_cache($topic_id);
        header('Location: forums.php?action=view_topic&topic_id=' . $topic_id);
        die();
        break;
