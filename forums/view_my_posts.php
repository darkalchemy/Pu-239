...
while ($arr = mysqli_fetch_assoc($res)) {
    ...
    $post_icon  = ($arr['icon'] !== '' ? '<img src="' . $site_config['pic_baseurl'] . 'smilies/' . htmlsafechars($arr['icon']) . '.gif" alt="icon" title="icon" /> ' : '<img src="' . $site_config['pic_baseurl'] . 'forums/topic_normal.gif" alt="Normal Topic" title="Normal Topic" /> ');
    ...
    $HTMLOUT .= '<tr><td colspan="3" align="left">' . $lang['fe_forum'] . ':
			...
			<td class="forum_head" align="right" valign="middle"><span style="white-space:nowrap;"> 
			 ...
			 <a href="forums.php?action=view_my_posts&amp;page=' . $page . '#bottom"><img src="' . $site_config['pic_baseurl'] . 'forums/down.gif" alt="' . $lang['fe_bottom'] . '" /></a> 
			</span></td>
			</tr>
			<tr>
		   <td class="' . $class_alt . '" width="100px" valign="top">' . ('yes' == $arr['anonymous'] ? '<img style="max-width:' . $width . 'px;" src="' . $site_config['pic_baseurl'] . 'anonymous_1.jpg" alt="avatar" />' : avatar_stuff($CURUSER)) . '<br>' . ('yes' == $arr['anonymous'] ? '<i>' . $lang['fe_anonymous'] . '</i>' : format_username($CURUSER['id'])) . ($arr['anonymous'] === 'yes' || $CURUSER['title'] === '' ? '' : '<br><span style=" font-size: xx-small;">[' . htmlsafechars($CURUSER['title']) . ']</span>') . '<br><span style="font-weight: bold;">' . ('yes' == $arr['anonymous'] ? '' : get_user_class_name($CURUSER['class'])) . '</span><br></td>
