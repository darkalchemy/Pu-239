...
while ($arr = mysqli_fetch_assoc($res)) {
    ...
    $HTMLOUT .= '<tr><td colspan="3" align="left">' . $lang['fe_forum'] . ':
			...
			<tr>
		   <td class="' . $class_alt . '" width="100px" valign="top">' . ('yes' == $arr['anonymous'] ? '<img src="' . $site_config['pic_baseurl'] . 'anonymous_1.jpg" alt="avatar" class="avatar">' : avatar_stuff($CURUSER)) . '<br>' . ('yes' == $arr['anonymous'] ? '<i>' . $lang['fe_anonymous'] . '</i>' : format_username($CURUSER['id'])) . ($arr['anonymous'] === 'yes' || $CURUSER['title'] === '' ? '' : '<br><span style=" font-size: xx-small;">[' . htmlsafechars($CURUSER['title']) . ']</span>') . '<br><span style="font-weight: bold;">' . ('yes' == $arr['anonymous'] ? '' : get_user_class_name($CURUSER['class'])) . '</span><br></td>
