...
while ($arr = mysqli_fetch_assoc($res)) {
    ...
    $HTMLOUT .= '<tr><td colspan="3" align="left">' . $lang['fe_forum'] . ':
			...
			<tr>
		   <td class="has-text-centered w-15 mw-150 ' . $class_alt . '" valign="top">' . get_avatar($CURUSER) . '<br>' . ($arr['anonymous'] === 'yes' ? '<i>' . get_anonymous_name() . '</i>' : format_username($CURUSER['id'])) . ($arr['anonymous'] === 'yes' || $CURUSER['title'] === '' ? '' : '<br><span style=" font-size: xx-small;">[' . htmlsafechars($CURUSER['title']) . ']</span>') . '<br><span style="font-weight: bold;">' . ($arr['anonymous'] === 'yes' ? '' : get_user_class_name($CURUSER['class'])) . '</span><br></td>
