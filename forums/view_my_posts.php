...
while ($arr = mysqli_fetch_assoc($res)) {
    ...
    $HTMLOUT .= '<tr><td class="forum_head_dark" colspan="3">' . $lang['fe_forum'] . ':
			...
			<tr>
		   <td>' . ($arr['anonymous'] == 'yes' ? '<img style="max-width:' . $width . 'px;" src="' . $site_config['pic_base_url'] . 'anonymous_1.jpg" alt="avatar" />' : avatar_stuff($CURUSER)) . '<br>' . ($arr['anonymous'] == 'yes' ? '<i>' . $lang['fe_anonymous'] . '</i>' : print_user_stuff($CURUSER)) . ($arr['anonymous'] == 'yes' || $CURUSER['title'] == '' ? '' : '<br><span>[' . htmlsafechars($CURUSER['title']) . ']</span>') . '<br><span>' . ($arr['anonymous'] == 'yes' ? '' : get_user_class_name($CURUSER['class'])) . '</span><br></td>
