   ...
   ORDER BY t.last_post DESC ' . $LIMIT) or sqlerr(__FILE__, __LINE__);
    ...
    while ($arr_unread = mysqli_fetch_assoc($res_unread)) {
        ...
        if ($arr_post_read[0] < $arr_unread['last_post'] && $posted) {
            ...
            if ($arr_unread['tan'] === 'yes') {
                if ($CURUSER['class'] < UC_STAFF && $arr_unread['user_id'] != $CURUSER['id']) {
                    ...
                    $thread_starter = (!empty($arr_unread['username']) ? '<i>' . get_anonymous_name() . '</i> [' . format_username((int) $arr_unread['id']) . ']' : '' . $lang['fe_lost'] . ' [' . $arr_unread['id'] . ']') . '<br>' . get_date((int) $first_unread_poster_arr[0], '');
                }
            } else {
                $thread_starter = (!empty($arr_unread['username']) ? format_username((int) $arr_unread['id']) : '' . $lang['fe_lost'] . ' [' . $arr_unread['id'] . ']') . '<br>' . get_date((int) $first_unread_poster_arr[0], '');
            }
		...
		<td>
    		...
    		<hr>in: <a class="altlink" href="' . $site_config['paths']['baseurl'] . '/forums.php?action=view_forum&amp;forum_id=' . (int) $arr_unread['forum_id'] . '">' . htmlsafechars($arr_unread['forum_name']) . '</a>
    		' . (!empty($arr_unread['topic_desc']) ? ' [ <span style="font-size: x-small;">' . htmlsafechars($arr_unread['topic_desc']) . '</span> ]' : '') . '
        </td>
