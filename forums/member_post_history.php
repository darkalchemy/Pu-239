...
if (!isset($member_id) || !is_valid_id($member_id)) {
    ...
    if ($arr_count[0] > 0) {
        ...
        while ($row = mysqli_fetch_assoc($res)) {
            ...
            $country = ($row['name'] != null) ? '<td class="' . $class . '"><img src="' . $site_config['pic_baseurl'] . 'flag/' . $row['flagpic'] . '" alt="' . htmlsafechars($row['name']) . '" /></td>' : '<td class="' . $class . '">---</td>';
            $HTMLOUT .= '<tr>
...
$page                = isset($_GET['page']) ? (int) $_GET['page'] : 0;
$perpage             = isset($_GET['perpage']) ? (int) $_GET['perpage'] : 20;
$subscription_on_off = (isset($_GET['s']) ? (1 == $_GET['s'] ? '<br><div style="font-weight: bold;">' . $lang['fe_sub_to_topic'] . ' <img src="' . $site_config['pic_baseurl'] . 'forums/subscribe.gif" alt=" " width="25"></div>' : '<br><div style="font-weight: bold;">' . $lang['fe_unsub_to_topic'] . ' <img src="' . $site_config['pic_baseurl'] . 'forums/unsubscribe.gif" alt=" " width="25"></div>') : '');
...
while ($arr = mysqli_fetch_assoc($res)) {
    ...
    switch ($post_status) {
        ...
        case 'recycled':
            ...
            $post_status_image = ' <img src="' . $site_config['pic_baseurl'] . 'forums/recycle_bin.gif" alt="' . $lang['fe_recycled'] . '" title="' . $lang['fe_this_post_is_currently'] . ' ' . $lang['fe_in_the_recycle_bin'] . '" width="24px" />';
            break;

        case 'deleted':
            $post_status       = 'deleted';
            $post_status_image = ' <img src="' . $site_config['pic_baseurl'] . 'forums/delete_icon.gif" alt="' . $lang['fe_deleted'] . '" title="' . $lang['fe_this_post_is_currently'] . ' ' . $lang['fe_deleted'] . '" width="24px" />';
            break;

        case 'postlocked':
            $post_status       = 'postlocked';
            $post_status_image = ' <img src="' . $site_config['pic_baseurl'] . 'forums/thread_locked.gif" alt="' . $lang['fe_locked'] . '" title="' . $lang['fe_this_post_is_currently'] . ' ' . $lang['fe_locked'] . '" width="24px" />';
            break;
    }
    $post_icon  = ($arr['icon'] !== '' ? '<img src="' . $site_config['pic_baseurl'] . 'smilies/' . htmlsafechars($arr['icon']) . '.gif" alt="icon" /> ' : '<img src="' . $site_config['pic_baseurl'] . 'forums/topic_normal.gif" alt="icon" /> ');
    ...
    $HTMLOUT .= '<tr>
		...
		<a href="forums.php?action=view_my_posts&amp;page=' . $page . '#top"><img src="' . $site_config['pic_baseurl'] . 'forums/up.gif" alt="' . $lang['fe_top'] . '" /></a> 
		<a href="forums.php?action=view_my_posts&amp;page=' . $page . '#bottom"><img src="' . $site_config['pic_baseurl'] . 'forums/down.gif" alt="' . $lang['fe_bottom'] . '" /></a></span></td>
		</tr>
		<tr>
		<td class="' . $class_alt . '" width="100px" valign="top">' . ($arr['anonymous'] === 'yes' ? '<img style="max-width:' . $width . 'px;" src="' . $site_config['pic_baseurl'] . 'anonymous_1.jpg" alt="avatar" />' : avatar_stuff($arr)) . '<br>' . ($arr['anonymous'] === 'yes' ? '<i>' . $lang['fe_anonymous'] . '</i>' : format_username($member_id)) . ($arr['anonymous'] === 'yes' || $user_arr['title'] === '' ? '' : '<br><span style=" font-size: xx-small;">[' . htmlsafechars($user_arr['title']) . ']</span>') . '<br><span style="font-weight: bold;">' . ($arr['anonymous'] === 'yes' ? '' : get_user_class_name($user_arr['class'])) . '</span><br></td>
