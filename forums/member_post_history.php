...
if (!isset($member_id) || !is_valid_id($member_id)) {
    ...
    if ($arr_count[0] > 0) {
        ...
        while ($row = mysqli_fetch_assoc($res)) {
            ...
            $country = ($row['name'] != null) ? '<td class="' . $class . '" align="center"><img src="./images/flag/' . $row['flagpic'] . '" alt="' . htmlsafechars($row['name']) . '" /></td>' : '<td class="' . $class . '" align="center">---</td>';
            $HTMLOUT .= '<tr>
...
$page = isset($_GET['page']) ? (int)$_GET['page'] : 0;
$perpage = isset($_GET['perpage']) ? (int)$_GET['perpage'] : 20;
$subscription_on_off = (isset($_GET['s']) ? ($_GET['s'] == 1 ? '<br><div style="font-weight: bold;">' . $lang['fe_sub_to_topic'] . ' <img src="./images/forums/subscribe.gif" alt=" " width="25"></div>' : '<br><div style="font-weight: bold;">' . $lang['fe_unsub_to_topic'] . ' <img src="./images/forums/unsubscribe.gif" alt=" " width="25"></div>') : '');
