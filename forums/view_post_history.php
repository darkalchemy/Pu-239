...
$res = sql_query(
    'SELECT p.added, p.body, p.edited_by, p.user_id AS poster_id, p.edit_date, p.post_title, p.icon, p.post_history, p.bbcode, p.anonymous, 
                  t.topic_name AS topic_name, f.name AS forum_name, u.id, u.username, u.class, u.donor, u.suspended, u.warned, u.enabled, u.avatar, 
                  u.offensive_avatar, u.chatpost, u.leechwarn, u.pirate, u.king
            FROM posts AS p 
            LEFT JOIN topics AS t ON p.topic_id = t.id 
            LEFT JOIN forums AS f ON t.forum_id = f.id 
            LEFT JOIN users AS u ON p.user_id = u.id 
            WHERE ' . ($CURUSER['class'] < UC_STAFF ? 'p.status = "ok" AND t.status = "ok" AND ' : ($CURUSER['class'] < $min_delete_view_class ? ' p.status != "deleted"
                 AND t.status != "deleted"  AND ' : '')) . ' p.id = ' . sqlesc($post_id)
...
$HTMLOUT .= '<h1>' . ($arr['anonymous'] == 'yes' ? '<i>' . $lang['fe_anonymous'] . '</h1>' : htmlsafechars($arr['username'])) . '\'s ' . $lang['vph_final_edit_post'] . '. ' . $lang['vph_last_edit_by'] . ': ' . ($arr['anonymous'] == 'yes' ? '<i>' . $lang['fe_anonymous'] . '</i>' : format_username($arr_edited)) . '</h1>
