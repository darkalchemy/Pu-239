...
while ($hit_and_run_arr = mysqli_fetch_assoc($hit_and_run_rez)) {
    ...
    if ($Xbt_Seed) {
        if ($Uid_ID !== $hit_and_run_arr['owner']) {
            ...
            $avatar = ($CURUSER['avatars'] == 'yes' ? ($hit_and_run_arr['avatar'] == '' ? '<img src="' . $site_config['pic_baseurl'] . 'forumicons/default_avatar.gif"  width="40" alt="default avatar" />' : '<img src="' . htmlsafechars(image_proxy($hit_and_run_arr['avatar'])) . '" alt="avatar"  width="40" />') : '');
