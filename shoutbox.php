...
if (count($shouts) > 0) {
    ...
    if ($shout_pm_alert['pms'] > 0 && $gotpm == 0) {
        $HTMLOUT.= '<tr><td align=\'center\'><a href=\'' . $INSTALLER09['baseurl'] . '/pm_system.php\' target=\'_parent\'><span style=\'color:red;\'>' . sprintf($lang['shoutbox_msg'], $shout_pm_alert['pms']) . ($shout_pm_alert['pms'] > 1 ? $lang['shoutbox_msg_s'] : "") .  '</span></a></td></tr>';
        $gotpm++;
    }
    if ($shouts) {
        if ($CURUSER['perms'] & bt_options::NOFKNBEEP) {
            if (preg_match(sprintf("/%s/iU", $CURUSER['username']) , $shouts[0]['text']) && ($shouts[0]['date'] - TIME_NOW) < 60) $HTMLOUT.= "<audio autoplay=\"autoplay\"><source src=\"templates/1/beep.mp3\" type=\"audio/mp3\" /><source src=\"templates/1/beep.ogg\" type=\"audio/ogg\" /></audio>";
        }
        //$HTMLOUT .= "<!-- ".print_r($shouts)."-->";
        foreach ($shouts as $arr) {
            if (($arr['to_user'] != $CURUSER['id'] && $arr['to_user'] != 0) && $arr['userid'] != $CURUSER['id']) continue;
            $private = '';
            if ($arr['to_user'] == $CURUSER['id'] && $arr['to_user'] > 0) $private = "<a href=\"javascript:private_reply('" . htmlsafechars($arr['username']) . "')\"><img src=\"{$INSTALLER09['pic_base_url']}private-shout.png\" alt=\"{$lang['shoutbox_priv_shout']}\" title=\"{$lang['shoutbox_priv_shout1']}" . htmlsafechars($arr['username']) . "\" width=\"16\" style=\"padding-left:2px;padding-right:2px;\" border=\"0\" /></a>";
            $edit = ($CURUSER['class'] >= UC_STAFF || ($arr['userid'] == $CURUSER['id']) && ($CURUSER['class'] >= UC_POWER_USER && $CURUSER['class'] <= UC_STAFF) ? "<a href='{$INSTALLER09['baseurl']}/shoutbox.php?edit=" . (int)$arr['id'] . "&amp;user=" . (int)$arr['userid'] . "'><img src='{$INSTALLER09['pic_base_url']}button_edit2.gif' border='0' alt=\"{$lang['shoutbox_edit_shout']}\"  title=\"{$lang['shoutbox_edit_shout']}\" /></a> " : "");
            $del = ($CURUSER['class'] >= UC_STAFF ? "<a href='./shoutbox.php?del=" . (int)$arr['id'] . "'><img src='{$INSTALLER09['pic_base_url']}button_delete2.gif' border='0' alt=\"{$lang['shoutbox_delete_shout']}\" title=\"{$lang['shoutbox_delete_shout']}\" /></a> " : "");
            $delall = ($CURUSER['class'] == UC_MAX ? "<a href='./shoutbox.php?delall' onclick=\"confirm_delete(); return false;\"><img src='{$INSTALLER09['pic_base_url']}del.png' border='0' alt=\"{$lang['shoutbox_empty_shout']}\" title=\"{$lang['shoutbox_empty_shout']}\" /></a> " : "");
            //$delall
            $pm = ($CURUSER['id'] != $arr['userid'] ? "<span class='date' style=\"color:$dtcolor\"><a target='_blank' href='./pm_system.php?action=send_message&amp;receiver=" . ($arr['perms'] & bt_options::PERMS_STEALTH ? "{$INSTALLER09['bot_id']}" : "" . (int)$arr['userid'] . "") . "'><img src='{$INSTALLER09['pic_base_url']}button_pm2.gif' border='0' alt=\"{$lang['shoutbox_pm_user']}\" title=\"{$lang['shoutbox_pm_user']}\" /></a></span>\n" : "");
            $date = get_date($arr["date"], 'TINY', 1);
            $reply = ($CURUSER['id'] != $arr['userid'] ? "<a href=\"javascript:window.top.SmileIT('[b][i]=>&nbsp;[color=#" . get_user_class_color($arr['class']) . "]" . ($arr['perms'] & bt_options::PERMS_STEALTH ? "UnKnown" : htmlsafechars($arr['username'])) . "[/color]&nbsp;-[/i][/b]','shbox','shbox_text')\"><img height='10' src='{$INSTALLER09['pic_base_url']}reply.gif' title='{$lang['shoutbox_reply']}' alt='{$lang['shoutbox_reply']}' style='border:none;' /></a>" : "");
            $user_stuff = $arr;
