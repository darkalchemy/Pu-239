    ...
    if ($shouts) {
        if ($CURUSER['perms'] & bt_options::NOFKNBEEP) {
            if (preg_match(sprintf('/%s/iU', $CURUSER['username']), $shouts[0]['text']) && ($shouts[0]['date'] - TIME_NOW) < 60) {
                $HTMLOUT .= '<audio autoplay="autoplay"><source src="templates/1/beep.mp3" type="audio/mp3" /><source src="templates/1/beep.ogg" type="audio/ogg" /></audio>';
            }
        }
        ...
        foreach ($shouts as $arr) {
            ...
            if ($arr['to_user'] == $CURUSER['id'] && $arr['to_user'] > 0) {
                $private = "<a href=\"javascript:private_reply('".htmlsafechars($arr['username'])."')\"><img src=\"{$INSTALLER09['pic_base_url']}private-shout.png\" alt=\"{$lang['shoutbox_priv_shout']}\" title=\"{$lang['shoutbox_priv_shout1']}".htmlsafechars($arr['username']).'" width="16" style="padding-left:2px;padding-right:2px;" border="0" /></a>';
            }
