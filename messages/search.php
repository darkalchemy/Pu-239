...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($as_list_post === 1) {
        ...
        while ($row = mysqli_fetch_assoc($res_search)) {
            ...
            $subject = str_ireplace($keywords, " < span style = 'background-color:yellow;font-weight:bold;color:black;' >{
    $keywords}</span > ", htmlsafechars($row['subject']));
            $table_body .= " < tr>
                <td class='w-10 has-text-centered' > $read</td >
                <td ><a href = '{$site_config['baseurl']}/messages.php?id={$row['id']}' >{
    $subject}</a ></td >
        ...
        while ($row = mysqli_fetch_assoc($res_search)) {
            ...
            $body = str_ireplace($keywords, " < span style = 'background-color:yellow;font-weight:bold;color:black;' >{
    $keywords}</span > ", format_comment($row['msg']));
            $subject = str_ireplace($keywords, " < span style = 'background-color:yellow;font-weight:bold;color:black;' >{
    $keywords}</span > ", htmlsafechars($row['subject']));
            $table .= main_table(" < tr>
