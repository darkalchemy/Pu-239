...
function bookmarktable($res, $variant = 'index')
{
   ...
   <td class='colhead text-left'>{$lang['torrenttable_name']}</td>";
    $htmlout .= ($variant == 'index' ? '<td class="colhead text-center">' . $lang['bookmarks_del2'] . '</td><td class="colhead text-right">' : '') . '' . $lang['bookmarks_down2'] . '</td><td class="colhead text-right">' . $lang['bookmarks_share'] . '</td>';
    if ($variant == 'mytorrents') {
        $htmlout .= "<td class='colhead text-center'>{$lang['torrenttable_edit']}</td>\n";
        $htmlout .= "<td class='colhead text-center'>{$lang['torrenttable_visible']}</td>\n";
    }
    $htmlout .= "<td class='colhead text-right'>{$lang['torrenttable_files']}</td>
   <td class='colhead text-right'>{$lang['torrenttable_comments']}</td>
   <td class='colhead text-center'>{$lang['torrenttable_added']}</td>
   <td class='colhead text-center'>{$lang['torrenttable_size']}</td>
   <td class='colhead text-center'>{$lang['torrenttable_snatched']}</td>
   <td class='colhead text-right'>{$lang['torrenttable_seeders']}</td>
   <td class='colhead text-right'>{$lang['torrenttable_leechers']}</td>";
    if ($variant == 'index') {
        $htmlout .= "<td class='colhead text-center'>{$lang['torrenttable_uppedby']}</td>\n";
    }
    $htmlout .= "</tr>\n";
    ...
    while ($row = mysqli_fetch_assoc($res)) {
        ...
        if ($variant == 'mytorrents') {
            $htmlout .= "</td><td class='text-center'><a href='edit.php?returnto=" . urlencode($_SERVER['REQUEST_URI']) . '&amp;id=' . (int)$row['id'] . "'>{$lang['torrenttable_edit']}</a>\n";
        }
        if ($variant == 'mytorrents') {
        ...
        $htmlout .= "<td class='text-center'><a href='snatches.php?id=$id'>" . number_format($row['times_completed']) . "<br>$_s</a></td>\n";
        if ((int)$row['seeders']) {
            if ($variant == 'index') {
                if ($row['leechers']) {
                ...
                $htmlout .= "<td class='text-right'><b><a href='peerlist.php?id=$id#seeders'><font color='" . get_slr_color($ratio) . "'>" . (int)$row['seeders'] . "</font></a></b></td>\n";
            } else {
                $htmlout .= "<td class='text-right'><b><a class='" . linkcolor($row['seeders']) . "' href='peerlist.php?id=$id#seeders'>" . (int)$row['seeders'] . "</a></b></td>\n";
            }
        } else {
        ...
        if ((int)$row['leechers']) {
            if ($variant == 'index') {
                $htmlout .= "<td class='text-right'><b><a href='peerlist.php?id=$id#leechers'>" . number_format($row['leechers']) . "</a></b></td>\n";
            } else {
                $htmlout .= "<td class='text-right'><b><a class='" . linkcolor($row['leechers']) . "' href='peerlist.php?id=$id#leechers'>" . (int)$row['leechers'] . "</a></b></td>\n";
            }
        } else {
            $htmlout .= "<td class='text-right'>0</td>\n";
        }
        if ($variant == 'index') {
            $htmlout .= "<td class='text-center'>" . (isset($row['username']) ? ("<a href='userdetails.php?id=" . (int)$row['owner'] . "'><b>" . htmlsafechars($row['username']) . '</b></a>') : '<i>(' . $lang['torrenttable_unknown_uploader'] . ')</i>') . "</td>\n";
        }
        $htmlout .= "</tr>\n";
    }
    $htmlout .= "</table>\n";
