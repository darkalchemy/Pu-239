...
(function($) {
    var iCarousel = function(el, slides, options) {
        ...
        if (ic.defs.total > 1 && ic.defs.timer != "bar") {
            ...
            var width = diameter + padding * 2 + stroke * 2, height = width, r = Raphael(ic.iCarouselTimer[0], width, height), R = diameter / 2, param = {
            ...
            r.customAttributes.arc = function(value, R) {
                var total = 360, alpha = 360 / total * value, a = (90 - alpha) * Math.PI / 180, cx = diameter / 2 + padding + stroke, cy = diameter / 2 + padding + stroke, x = cx + R * Math.cos(a), y = cy - R * Math.sin(a), path;
                if (total == value) {
    ...
    iCarousel.prototype = {
        ...
        layout: function(animate, speedTime) {
            var ic = this;
            ic.setItems();
            var slideTop = ic.defs.topSpace == "auto" ? ic.defs.height / 2 - ic.defs.currentSlide.data("outerheight") / 2 : 0, slideLeft = ic.defs.width / 2 - ic.defs.currentSlide.data("outerwidth") / 2, center = ic.defs.width / 4, zIndex = 999, css = {}, anim = {}, speed = speedTime ? speedTime / 1e3 : ic.options.animationSpeed / 1e3;
        ...
        newDimenstions: function(width, height, width_old, height_old) {
            if (width == 0) factor = height / height_old; else if (height == 0) factor = width / width_old; else factor = Math.min(width / width_old, height / height_old);
        ...
        CSS: function(slide, i, zIndex, positive) {
            var ic = this, leftRemain = ic.defs.space == "auto" ? parseInt((i + 1) * (slide.data("width") / 1.5)) : parseInt((i + 1) * ic.defs.space);
            if (ic.support.transform3d() && ic.options.make3D) {
                var transform = positive ? "translateX(" + leftRemain + "px) translateZ(-" + (250 + (i + 1) * 110) + "px) rotateY(-" + ic.options.perspective + "deg)" : "translateX(-" + leftRemain + "px) translateZ(-" + (250 + (i + 1) * 110) + "px) rotateY(" + ic.options.perspective + "deg)", left = "0%", top = ic.defs.topSpace == "auto" ? "none" : parseInt((i + 1) * ic.defs.space), width = "none", height = "none", overflow = "visible";
            } else if (ic.support.transition() && ic.support.transform2d()) {
                var transform = positive ? "translateX(" + leftRemain / 1.5 + "px) scale(" + (1 - i / 10 - .1) + ")" : "translateX(-" + leftRemain / 1.5 + "px) scale(" + (1 - i / 10 - .1) + ")", left = "0%", top = ic.defs.topSpace == "auto" ? "none" : parseInt((i + 1) * ic.defs.topSpace), width = "none", height = "none", overflow = "visible";
            } else {
                var wDiff = slide.data("outerwidth") - slide.data("width"), hDiff = slide.data("outerheight") - slide.data("height"), dims = ic.newDimenstions(slide.data("width") * (1 - i / 10 - .1), slide.data("height") * (1 - i / 10 - .1), slide.data("width"), slide.data("height")), leftRemain = ic.defs.space == "auto" ? parseInt((i + 1) * (dims.width / 1.5)) : parseInt((i + 1) * ic.defs.space), transform = "", left = positive ? ic.defs.width / 2 - (dims.width + hDiff) / 2 - leftRemain / 1.5 + "px" : ic.defs.width / 2 - (dims.width + hDiff) / 2 + leftRemain / 1.5 + "px", width = dims.width, height = dims.height, center = ic.defs.height / 2 - (dims.height + hDiff) / 2, top = ic.defs.topSpace == "auto" ? center : parseInt(center + i * ic.defs.topSpace), overflow = "hidden";
            }
