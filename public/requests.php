...
c.id AS cat_id, c.name AS cat_name, c.image AS cat_image FROM requests AS r LEFT JOIN categories AS c ON r.category = c.id LEFT JOIN users AS u ON r.requested_by_user_id = u.id ORDER BY r.added DESC ' . $LIMIT);
    ...
    <tr>
        ...
        while ($main_query_arr = mysqli_fetch_assoc($main_query_res)) {
            ...
            $HTMLOUT .= '
    <tr>
        <td><img src="' . $site_config['pic_baseurl'] . 'caticons/' . get_categorie_icons() . '/' . htmlsafechars($main_query_arr['cat_image'], ENT_QUOTES) . '" alt="' . htmlsafechars($main_query_arr['cat_name'], ENT_QUOTES) . '" /></td>
  ...
  <tr>
  <td>category:</td>
  <td><img src="' . $site_config['pic_baseurl'] . 'caticons/' . get_categorie_icons() . '/' . htmlsafechars($arr['cat_image'], ENT_QUOTES) . '" alt="' . htmlsafechars($arr['cat_name'], ENT_QUOTES) . '" /></td>
  </tr>
  <tr>
  <td>link:</td>
