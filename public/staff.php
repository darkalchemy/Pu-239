        ...
        for ($i = 0; $i < $cols; ++$i) {
            if (isset($staff[$r])) {
                $htmlout .= "
                        ...
                        <td class='staff_online'><img src='{$site_config['pic_baseurl']}staff" . ($staff[$r]['last_access'] > $dt && $staff[$r]['perms'] < bt_options::PERMS_STEALTH ? '/online.png' : '/offline.png') . "' height='16' alt='' /></td>" . "
                        <td class='staff_online'><a href='{$site_config['baseurl']}/pm_system.php?action=send_message&amp;receiver=" . (int)$staff[$r]['id'] . '&amp;returnto=' . urlencode($_SERVER['REQUEST_URI']) . "'><img src='{$site_config['pic_baseurl']}mailicon.png' class='tooltipper' title='Personal Message' alt='' /></a></td>" . "
                        <td class='staff_online'><img height='16' src='{$site_config['pic_baseurl']}flag/" . htmlsafechars($staff[$r]['flagpic']) . "' alt='" . htmlsafechars($staff[$r]['name']) . "' /></td>";
                ++$r;
            } else {
                $htmlout .= '<td></td>';
            }
