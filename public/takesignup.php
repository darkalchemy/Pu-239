...
$psecret = '';
if ($users_count > 0 && $site_config['signup']['email_confirm']) {
...
if ($user_id > 2 && ($site_config['site']['autoshout_chat'] || $site_config['site']['autoshout_irc'])) {
    $message = "Welcome New {$site_config['site']['name']} Member: [user]" . htmlsafechars($wantusername) . '[/user]';
    autoshout($message);
}
...
if ($users_count > 0 && $site_config['signup']['email_confirm']) {
    $body = str_replace([
        ...
        $email,
        $ip,
        "{$site_config['paths']['baseurl']}/confirm.php?id=$alt_id$psecret",
...
if ($site_config['site']['auto_confirm']) {
    $user_stuffs->delete_user_cache([$user_id]);
    $session->set('userID', $user_id);
}

$session->unset('signup_variables');
header("Location: {$site_config['paths']['baseurl']}/ok.php?type=" . ($users_count === 0 ? 'sysop' : ($site_config['signup']['email_confirm'] ? 'signup&email=' . urlencode($email) : 'confirm')));
