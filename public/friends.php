...
if (mysqli_num_rows($res) == 0) {
    ...
    while ($friendp = mysqli_fetch_assoc($res)) {
        ...
        $linktouser = format_username((int) $friendp['id']) . " [$title]<br>{$lang['friends_last_seen']} " . (get_anonymous((int) $friendp['id']) ? get_date((int) $friendp['last_access'], '') : 'Never');
...
if (mysqli_num_rows($res) == 0) {
    ...
    while ($friend = mysqli_fetch_assoc($res)) {
        ...
        $linktouser = format_username((int) $friend['id']) . " [$title] [$ratio]<br>{$lang['friends_last_seen']} " . (get_anonymous((int) $friend['id']) ? get_date((int) $friend['last_access'], '') : 'Never');
