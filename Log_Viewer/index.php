...
<head>
    ...
    <title><?php echo TITLE; ?></title><?php include_once 'inc/favicon.inc.php'; ?>
    ...
    <script>var logs_refresh_default = <?php echo (int)LOGS_REFRESH; ?>,
        ...
        notification_default = <?php echo (NOTIFICATION === true) ? 'true' : 'false'; ?>;</script>
</head>
<body><!--[if lt IE 8]><p class="chromeframe">You are using an <strong>outdated</strong> browser. Please <a
        ...
        <div class="navbar-header">
            ...
            <div class="navbar-brand"><span class="loader glyphicon glyphicon-refresh icon-spin"
                                            style="display:none"></span> <span class="loader glyphicon glyphicon-repeat"
                                                                               title="<?php _h('Click to refresh or press the R key'); ?>"
                                                                               id="refresh"></span> <a
                        href="?"><?php echo NAV_TITLE; ?></a></div>
        </div>
        <div class="navbar-collapse collapse"><?php if (count($files) > 1) : ?><?php if (FILE_SELECTOR == 'bs') : ?>
                <ul class="nav navbar-nav">
                <li class="dropdown" title="<?php _h('Select a log file to display'); ?>"><a href="#"
                                                                                             class="dropdown-toggle"
                                                                                             data-toggle="dropdown"><span
                                id="file_selector"></span></a>
                ...
                <div class="form-group"><select id="file_selector_big" class="form-control input-sm"
                                                title="<?php _h('Select a log file to display'); ?>"><?php
            ...
            <form class="navbar-form navbar-right"><?php if ((is_null($current_user)) && (Sentinel::isAnonymousEnabled($files))) {
                    ?>
                ...
                <div class="form-group" id="searchctn"><input type="text" class="form-control input-sm clearable"
                    ...
                    <button style="display:none" type="button" id="notification" class="btn-menu btn-sm"
                            title="<?php _h('Desktop notifications on supported modern browsers'); ?>"><span
                                ...
                                class="visible-xs-* visible-sm-* hidden-md hidden-lg"><?php _e('Notifications'); ?></span>
                    </button>
                </div>
            </form>
            <ul class="nav navbar-nav navbar-right">
                ...
                <li class="dropdown"><a href="#" class="dropdown-toggle" data-toggle="dropdown"><span
                    ...
                    <ul class="dropdown-menu cogmenu" style="padding: 15px">
                        ...
                        <li><select id="cog-lang" class="form-control input-sm"
                                    ...
                                    <option value=""><?php _e('Change language...'); ?></option><?php
                                    ...
                                    ?><?php else : ?>
                                    <option value=""><?php _e('Language cannot be changed'); ?></option><?php endif; ?>
                            </select></li>
                        <li><select id="cog-tz" class="form-control input-sm" title="<?php _h('Timezone'); ?>">
                                <option value=""><?php _e('Change timezone...'); ?></option><?php
                ...
                </li><?php if (!is_null($current_user)) {
                    ?>
                    <li class="dropdown"><a href="#" class="dropdown-toggle" data-toggle="dropdown"
                                            title="<?php echo h(sprintf(__('You are currently connected as %s'), $current_user)); ?>"><span
                                class="glyphicon glyphicon-user"></span> <span
                                class="visible-xs-* visible-sm-* hidden-md hidden-lg"><?php _h('User settings'); ?></span></a>
                    <ul class="dropdown-menu"><?php if (Sentinel::isAdmin()) {
                            ?>
...
</div><?php if (PULL_TO_REFRESH === true) {
    ?>
    <div id="hook" class="hook">
        <div id="loader" class="hook-loader">
            <div class="hook-spinner"></div>
        </div>
        <span id="hook-text"></span></div><?php
} ?>
<div class="container"><?php if (isset($_SESSION['upgradegitpullok'])) :
        ...
        <div class="alert alert-success alert-dismissable">
            ...
            </button> <?php echo $print; ?></div></div><?php endif; ?>
    <div id="upgradeerrorctn" style="display:none"><br>
        <div class="alert alert-danger" id="upgradeerror"></div>
    </div>
    <div id="error" style="display:none"><br>
        <div class="alert alert-danger fade in"><h4>Oups!</h4>
            <p id="errortxt"></p></div>
    </div>
    <div class="result"><br>
    ...
    <div class="row" id="export" style="display:none">
        <div class="col-xs-8 col-xs-offset-2 col-sm-5 col-sm-offset-1 col-md-4 col-md-offset-2 col-lg-2 col-lg-offset-4">
            <button style="width:100%; margin-bottom:1em" type="button" class="loadmore btn btn-xs btn-primary"
                    ...
                    data-nomore-text="<?php _h('No more data'); ?>"><?php _e('Load more'); ?></button>
        </div>
            ...
            <div class="modal-body"><h3><?php _e('Webservice URL'); ?></h3>
                ...
                <div class="row">
                    <div class="col-xs-8"><a
                                class="btn btn-xs btn-primary clipboardex"><?php _e('Copy to clipboard'); ?></a></div>
                    <div class="col-xs-4 text-right"><a class="btn btn-xs btn-primary" id="exModalOpen"
                                                        target="_blank"><?php _e('Open'); ?></a></div>
                </div>
                <br>
                <blockquote><p><?php _e('Feel free to change these parameters in the URL:'); ?></p>
                    <ul>
                        ...
                        <li><code>timeout</code>
                            : <?php _e('the timeout in seconds to return log lines. Increase this value for big counts or when using search'); ?>
                        </li>
                        <li><code>search</code>
                            : <?php _e('search this value in log lines. It can be a regular expression or a regex one'); ?>
                        </li>
                        <li>
                            <code>callback</code>: <?php _e('this field is optional and is used to specify a callback function when format is JSONP'); ?>
                        </li>
                    </ul>
                </blockquote>
                ...
                <div id="exModalResult" style="display:none">
                    ...
                    <div class="row">
                        <div class="col-xs-8" style="margin-top:0"><a
                                    class="btn btn-xs btn-primary clipboardexr"><?php _e('Copy to clipboard'); ?></a>
                        </div>
                        <div class="col-xs-4 text-right">
                            <button class="btn btn-xs btn-primary" id="exModalRefresh"
                                    data-loading-text="<?php _h('Loading...'); ?>"
                                    onclick="refresh_rss()"><?php _e('Refresh'); ?></button>
                        </div>
                    </div>
                </div>
            </div>
...
<div class="modal fade" id="changeLogModal" tabindex="-1" role="dialog" aria-labelledby="exModalLabel"
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span
                            class="sr-only"><?php _e('Close'); ?></span></button>
                <h4 class="modal-title"><?php _e('Change Log'); ?></h4></div>
...
</div><?php if (!is_null($current_user)) {
    ?>
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal"><span
                                aria-hidden="true">&times;</span><span class="sr-only"><?php _e('Close'); ?></span>
                    </button>
                    <h4 class="modal-title" id="cpModalLabel"><?php _e('Change password'); ?></h4></div>
                <div class="modal-body">
                    <div class="alert alert-danger" role="alert" id="cpErr" style="display:none">
                        <strong><?php _e('Form is invalid:'); ?></strong>
                        <ul id="cpErrUl"></ul>
                    </div><?php echo sprintf(__('You are currently connected as %s'), '<code>' . $current_user . '</code>'); ?>
                    <br><br>
                    <div class="container">
                        <div class="row">
                            <div class="input-group col-sm-6 col-md-4" id="password1group"><span
                                        class="input-group-addon"><span
                                            class="glyphicon glyphicon-lock"></span></span><input type="password"
                                                                                                  ...
                                                                                                  name="password1"
                                                                                                  class="form-control"
                                                                                                  placeholder="<?php _h('Current password') ?>">
                            </div>
                            <br></div>
                        <div class="row">
                            <div class="input-group col-sm-6 col-md-4" id="password2group"><span
                                        class="input-group-addon"><span
                                            class="glyphicon glyphicon-lock"></span></span><input type="password"
                                                                                                  ...
                                                                                                  name="password2"
                                                                                                  class="form-control"
                                                                                                  placeholder="<?php _h('New password') ?>">
                            </div>
                            <br></div>
                        <div class="row">
                            <div class="input-group col-sm-6 col-md-4" id="password3group"><span
                                        class="input-group-addon"><span
                                            class="glyphicon glyphicon-lock"></span></span><input type="password"
                                                                                                  ...
                                                                                                  name="password3"
                                                                                                  class="form-control"
                                                                                                  placeholder="<?php _h('New password confirmation') ?>">
                            </div>
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal"><?php _e('Close'); ?></button>
                    <input type="submit" class="btn btn-primary" data-loading-text="<?php _h('Saving...'); ?>"
        ...
        <form id="prForm" autocomplete="off">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="prModalLabel"><?php _e('Profile'); ?></h4></div>
    ...
    </div></div><?php if (Sentinel::isAdmin()) {
        ?>
            ...
            <div class="modal-content" id="usermanagement">
                <div class="modal-header">
                    ...
                    <ul class="nav nav-pills">
                        <li class="active"><a href="#umUsers" role="tab" data-toggle="pill"><?php _e('Users'); ?></a>
                        </li>
                        <li><a href="#umAnonymous" role="tab" data-toggle="pill"><?php _e('Anonymous access'); ?></a>
                        </li>
                ...
                <div class="tab-content">
                    <div class="tab-pane active" id="umUsers">
                        ...
                        <div id="umUsersView" style="display:none">
                            ...
                            <div class="modal-footer">
                                <button type="button" class="btn btn-default"
                                        onclick="users_list()"><?php _e('« Back'); ?></button>
                                <button type="button" class="btn btn-primary" onclick="users_edit(this)"
                                        id="umUserEditBtn"><?php _e('Edit'); ?></button>
                            </div>
                        </div>
                            ...
                            <form id="umUsersAddForm" autocomplete="off" role="form">
                                ...
                                <div class="modal-body form-horizontal" id="umUsersAddBody">
                                    ...
                                    <div class="form-group"><label for="roles"
                                                                   class="col-sm-4 control-label"><?php _e('Roles'); ?></label>
                                        <div class="col-sm-8">
                                            <div class="btn-group" data-toggle="buttons"><label
                                                        ...
                                                        id="add-roles-user"><input type="radio" name="roles"
                                                                                   value="user"
                                                                                   checked="checked"><?php _e('User'); ?>
                                                </label><label class="btn btn-default btn-xs roles-admin"
                                                               id="add-roles-admin"><input type="radio" name="roles"
                                                                                           value="admin"><?php _e('Admin'); ?>
                                                </label></div>
                                        </div>
                                    </div>
                                    <div class="logs-selector">
                                        <div class="form-group"><label class="col-sm-4 control-label"></label>
                                            <div class="col-sm-8"><?php _e('Select which log files user can view'); ?>
                                                (<a href="#"
                                                    class="logs-selector-toggler"><?php _e('Toggle all log files'); ?></a>)
                                            </div>
                                        </div><?php foreach ($files as $file_id => $file) {
                                            $fid = h($file_id);
                                ...
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-default" onclick="users_view(this)"
                                            id="umUsersViewBtn"><?php _e('« Back'); ?></button>
                                    <button type="button" class="btn btn-default" onclick="users_list()"
                                            id="umUsersAddBtn"><?php _e('Cancel'); ?></button>
                                    <input type="submit" class="btn btn-primary"
                                           data-loading-text="<?php _h('Saving...'); ?>" value="<?php _h('Save'); ?>"
                                           id="umUsersAddSave"></div>
                                <input type="hidden" name="add-type" id="add-type" value="add"></form>
                        </div>
                    </div>
...
<script>numeral.language('<?php echo $localejs; ?>');</script><?php if (('UA-XXXXX-X' != GOOGLE_ANALYTICS) && ('' != GOOGLE_ANALYTICS)) {
    ?>
    <script>var _gaq = [['_setAccount', '<?php echo GOOGLE_ANALYTICS; ?>'], ['_trackPageview']];
        (function (d, t) {
