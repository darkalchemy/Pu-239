...
function achievement_seedtime_update($data)
{
    ...
    $res = sql_query('SELECT s.userid, s.seedtime, a.dayseed FROM snatched AS s LEFT JOIN usersachiev AS a ON s.userid = a.userid WHERE seedtime >= ' . sqlesc($seedtime) . ' GROUP BY userid') or sqlerr(__FILE__, __LINE__);
