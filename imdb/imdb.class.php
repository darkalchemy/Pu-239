...
/* $Id: imdb.class.php 632 2013-12-01 17:35:28Z izzy $ */
 ...
 *
 * @version $Revision: 632 $ $Date: 2013-12-01 18:35:28 +0100 (So, 01. Dez 2013) $
 */
class imdb extends movie_base
{
    ...
    public function __construct($id)
    {
        ...
        $this->revision = preg_replace('|^.*?(\d+).*$|', '$1', '$Revision: 632 $');
        $this->setid($id);
    }
    ...
    public function runtimes()
    {
        if (empty($this->movieruntimes)) {
            ...
            if (preg_match_all("/[\/ ]*((\D*?):|)([\d]+?) min( \((.*?)\)|)/", $this->main_runtime, $matches)) {
                for ($i = 0; $i < count($matches[0]); ++$i) {
                    $this->movieruntimes[] = ['time' => $matches[3][$i], 'country' => $matches[2][$i], 'comment' => $matches[5][$i]];
                }
            } elseif (preg_match('!<div class="infobar">.*?(\d+)\s*min!ims', $this->page['Title'], $match)) {
                $this->movieruntimes[] = ['time' => $match[1], 'country' => '', 'comment' => ''];
            }
        }
        ...
        if (empty($this->split_plot)) {
            ...
            for ($i = 0; $i < count($this->plot_plot); ++$i) {
                if (preg_match('!(.*?)<em>Written by\s+<a href="(.*?)"\s*>(.*?)</a>\s*</span>\s*</em>!ims', $this->plot_plot[$i], $match)) {
                    $this->split_plot[] = ['plot' => $match[1], 'author' => ['name' => $match[3], 'url' => $match[2]]];
                }
            }
        }
        ...
        if (empty($this->season_episodes)) {
            ...
            if (preg_match('!<select id="bySeason"(.*?)</select!ims', $this->page['Episodes'], $match)) {
                ...
                for ($i = 0; $i < count($matches[0]); ++$i) {
                    ...
                    $ec = count($eps[0]);
                    for ($ep = 0; $ep < $ec; ++$ep) {
                        $this->season_episodes[$s][$eps['episodeNumber'][$ep]] = ['imdbid' => $eps['imdbid'][$ep], 'title' => trim($eps['title'][$ep]), 'airdate' => $eps['airdate'][$ep], 'plot' => trim($eps['plot'][$ep]), 'season' => $s, 'episode' => $eps['episodeNumber'][$ep]];
                    }
                }
            }
            $this->imdbID = $tid;
                ...
                for ($i = 0; $i < $mc; ++$i) {
                    ...
                    if (preg_match_all('|^\s*(.*?)\s+by\s+(<a href[^>]+>.+?</a>)|i', $matches['desc'][$i], $match1)) {
                        for ($k = 0; $k < count($match1[0]); ++$k) {
                            switch ($match1[1][$k]) {
                                case 'Arranged':
                                    $s['credits'][] = ['credit_to' => str_replace('href="/', 'href="http://' . $this->imdbsite . '/', $match1[2][$k]), 'desc' => 'arrangement'];
                                    break;
                                case 'Composed':
                                    $s['credits'][] = ['credit_to' => str_replace('href="/', 'href="http://' . $this->imdbsite . '/', $match1[2][$k]), 'desc' => 'composer'];
                                    break;
                                case 'Performed':
                                ...
                                case 'Written and Performed': {
                                    ...
                                    break;
                                default:
                                    $s['credits'][] = ['credit_to' => str_replace('href="/', 'href="http://' . $this->imdbsite . '/', $match1[2][$k]), 'desc' => '**' . $match1[1][$k] . '**'];
                            }
                        }
        ...
        if (empty($this->release_info)) {
            ...
            for ($i = 0; $i < $mc; ++$i) {
                ...
                if (preg_match('!href="/date/(\d{2})-(\d{2})/">\d+ (.*?)</a>\s*<a href="/year/(\d{4})/">!is', $matches[2][$i], $match)) { // full info
                    ...
                    $this->release_info[] = ['country' => $country, 'day' => $match[1], 'month' => $match[2], 'mon' => $this->monthNo(trim($match[2])), 'year' => $match[3], 'comment' => $matches[3][$i]];
        ...
        if (empty($this->awards)) {
            ...
            for ($i = 0; $i < $rowcount; ++$i) {
                ...
                if (count($cels) == 1 && preg_match('|([^<>]*)<br>(.*)<small>|s', $cels[0], $matches)) {
                    ...
                    $people = isset($matches[0][0]) ? array_combine($matches[1], $matches[2]) : [];
                ...
                if (count($cels) == 0) {
                    $this->awards[$festival]['entries'][$nr] = [
