         ...
         if (empty($this->soundtracks)) {
             ...
             if (preg_match_all('!class="soundTrack soda (odd|even)"\s*>\s*(?<title>.+?)<br\s*/>(?<desc>.+?)</div>!ims', str_replace("\n", ' ', $this->page['Soundtrack']), $matches)) {
                ...
                case 'Arranged': foreach ($creds as $cred) {
                    $s['credits'][] = array('credit_to' => str_replace('href="/', 'href="http://'.$this->imdbsite.'/', $cred), 'desc' => 'arrangement');
                } break;
                case 'Composed': foreach ($creds as $cred) {
                    $s['credits'][] = array('credit_to' => str_replace('href="/', 'href="http://'.$this->imdbsite.'/', $cred), 'desc' => 'composer');
                } break;
                ...
                default: foreach ($creds as $cred) {
                    $s['credits'][] = array('credit_to' => str_replace('href="/', 'href="http://'.$this->imdbsite.'/', $cred), 'desc' => '**'.$match1[1][$k].'**');
                } break;
              }
                         }
                     }
