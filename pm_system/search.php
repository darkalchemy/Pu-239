...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($as_list_post === 1) {
        ...
        while ($row = mysqli_fetch_assoc($res_search)) {
            $read = $row['unread'] === 'yes' ? "<img src='{$site_config['baseurl']}/images/pn_inboxnew.gif' title='{$lang['pm_mailbox_unreadmsg']}' alt='{$lang['pm_mailbox_unread']}' class='tooltipper' />" : "<img src='{$site_config['baseurl']}/images/pn_inbox.gif title='{$lang['pm_mailbox_readmsg']}' alt='{$lang['pm_mailbox_read']}' class='tooltipper' />";
