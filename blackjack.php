...
$sql = "SELECT cards FROM {$blackjack['version']} WHERE userid != ".sqlesc($CURUSER['id']);
$res = sql_query($sql) or sqlerr(__FILE__, __LINE__);
...
if (count($list) > 0) {
    ...
    foreach ($player_cards as $card) {
        ...
        if ($card != $player_cards[0]) {
            $player_showcards .= "<img src='{$INSTALLER09['pic_base_url']}cards/".htmlsafechars($arr['pic'])."' width='71' height='96' border='0' alt='{$lang['bj_cards']}' title='{$lang['bj_cards']}' class='tooltipper padded' />";
        } else {
            $player_showcards .= "<img src='{$INSTALLER09['pic_base_url']}cards/back.jpg' width='71' height='96' border='0' alt='' alt='{$lang['bj_cards']}' title='{$lang['bj_cards']}' class='tooltipper padded' />";
        }
        $player_showcards_end .= "<img src='{$INSTALLER09['pic_base_url']}cards/".htmlsafechars($arr['pic'])."' width='71' height='96' border='0' alt='{$lang['bj_cards']}' title='{$lang['bj_cards']}' class='tooltipper padded' />";
    }
    $dealer = true;
    $user_warning = 'You are the dealer, you must take a card below 17.';
} else {
    $sql = "SELECT dealer_cards FROM {$blackjack['version']} WHERE userid = ".sqlesc($CURUSER['id']);
    $res = sql_query($sql) or sqlerr(__FILE__, __LINE__);
    $list = mysqli_fetch_assoc($res);
    if (count($list) > 0) {
        ...
        foreach ($dealer_cards as $card) {
            ...
            if ($card != $dealer_cards[0]) {
                $player_showcards .= "<img src='{$INSTALLER09['pic_base_url']}cards/".htmlsafechars($arr['pic'])."' width='71' height='96' border='0' border='0' alt='' alt='{$lang['bj_cards']}' title='{$lang['bj_cards']}' class='tooltipper padded' />";
            } else {
                $player_showcards .= "<img src='{$INSTALLER09['pic_base_url']}cards/back.jpg' width='71' height='96' border='0' alt='' alt='{$lang['bj_cards']}' title='{$lang['bj_cards']}' class='tooltipper padded' />";
            }
        }
...
$HTMLOUT .= "
		<section style='width: 650px; max-width: 90%;'>";
...
if ($game) {
    ...
    if ($start_ != 'yes') {
        $playeres = sql_query("SELECT b.* FROM {$blackjack['version']} AS b WHERE b.userid = ".sqlesc($CURUSER['id'])) or sqlerr(__FILE__, __LINE__);
        ...
        foreach ($arr as $card_id) {
            ...
            $showcards .= "<img src='{$INSTALLER09['pic_base_url']}cards/".htmlsafechars($used_cards['pic'])."' width='71' height='96' border='0' alt='{$lang['bj_cards']}' title='{$lang['bj_cards']}' class='tooltipper padded' />";
            ++$numCards;
    ...
    if ($_POST['game'] == 'hit') {
        if ($start_ == 'yes') {
            ...
            $res = sql_query("SELECT c.* FROM {$blackjack['version']} AS c WHERE c.userid = ".sqlesc($CURUSER['id'])) or sqlerr(__FILE__, __LINE__);
            ...
            if ($arr['status'] == 'waiting') {
                ...
                stderr($lang['bj_sorry'], "{$lang['bj_you_most_finish_current']}
					<form method='post' action='".$_SERVER['PHP_SELF']."?id=$id'>
						...
						<input class='btn-clean' type='submit' value='{$lang['bj_continue_old_game']}' />
					</form>");
            }
            ...
            if (!$dealer) {
                ...
                $player_showcards .= "<img src='{$INSTALLER09['pic_base_url']}cards/".htmlsafechars($card_details['pic'])."' width='71' height='96' border='0' border='0' alt='' alt='{$lang['bj_cards']}' title='{$lang['bj_cards']}' class='tooltipper padded' />";
                $player_showcards_end = $player_showcards;
            } else {
                $sql = "SELECT cards, dealer_cards FROM {$blackjack['version']} WHERE userid != ".sqlesc($CURUSER['id']);
                $res = sql_query($sql) or sqlerr(__FILE__, __LINE__);
                $list = mysqli_fetch_assoc($res);
            ...
            foreach ($cardids as $cardid) {
                ...
                $showcards .= "<img src='{$INSTALLER09['pic_base_url']}cards/".htmlsafechars($cardarr['pic'])."' width='71' height='96' border='0' alt='{$lang['bj_cards']}' title='{$lang['bj_cards']}' class='tooltipper padded' />";
                ++$numCards;
                $cardids2[] = $cardid;
            ...
            $sql = "INSERT INTO {$blackjack['version']} (userid, points, cards, date, dealer_cards) VALUES (".
                        sqlesc($CURUSER['id']).', '.
                        sqlesc($points).', '.
            ...
            if ($points < 21) {
				...
				<table class='table table-bordered text-center'>
					<tr>
						...
						<td class='text-center' style='width:50%;'>".trim($showcards)."</td>
					</tr>
					<tr>
                ...
                if (!$ddown) {
                    $HTMLOUT .= "
								<form method='post' action='".$_SERVER['PHP_SELF']."?id=$id'>
									<input type='hidden' name='game' value='hit' readonly='readonly' />
									<input class='btn-clean' type='submit' value='Hitme' />
								</form>";
                }
                if ($points >= 17 && $dealer || $points >= 10 && !$dealer) {
                    $HTMLOUT .= "
								<form method='post' action='".$_SERVER['PHP_SELF']."?id=$id'>
									...
									<input class='btn-clean' type='submit' value='{$lang['bj_stay']}' />
								</form>";
                }
                if ($points >= 9 && $points <= 11 && $numCards === 2 && !$dealer) {
                    $HTMLOUT .= "
								<form method='post' action='".$_SERVER['PHP_SELF']."?id=$id'>
									...
									<input type='hidden' name='game' value='hit' readonly='readonly' />
									<input class='btn-clean' type='submit' value='Double Down' />
								</form>";
                }
                ...
                //echo stdhead($blackjack['title']) . $HTMLOUT . stdfoot();
                //exit();
            }
            ...
            sql_query("UPDATE {$blackjack['version']} SET $update_ddown, points = ".sqlesc($points).", cards = '".$cards.' '.$cardid."' WHERE userid = ".sqlesc($CURUSER['id'])) or sqlerr(__FILE__, __LINE__);
        }
        if ($points == 21 || $points > 21) {
            $waitres = sql_query("SELECT COUNT(userid) AS c FROM {$blackjack['version']} WHERE status = 'waiting' AND userid != ".sqlesc($CURUSER['id'])) or sqlerr(__FILE__, __LINE__);
			...
			<table class='table table-bordered text-center'>
				<tr>
					<td class='text-center' style='width:50%;'>{$player_showcards_end}</td>
					<td class='text-center' style='width:50%;'>{$showcards}</td>
				</tr>
				<tr>
					<td class='text-center' style='width:50%;'>{$userName}</td>
					<td class='text-center' style='width:50%;'>".format_username($CURUSER)."<br>{$lang['bj_points']} = {$points}<br>{$user_warning}</td>
        ...
        if ($points == 21) {
            if ($waitarr['c'] > 0) {
                $r = sql_query("SELECT b.*, u.username, u.uploaded, u.downloaded, u.bjwins, u.bjlosses, u.class, u.gender, u.id, u.donor, u.warned, u.leechwarn, u.enabled, u.chatpost, u.pirate, u.king FROM {$blackjack['version']} AS b LEFT JOIN users AS u ON u.id=b.userid WHERE b.status='waiting' AND b.userid != ".sqlesc($CURUSER['id']).' ORDER BY b.date ASC LIMIT 1') or sqlerr(__FILE__, __LINE__);
                $a = mysqli_fetch_assoc($r);
                $points_text = htmlsafechars($a['points'])." {$lang['bj_points2']}";
                $card_count = count(explode(' ', $a['cards']));
                //file_put_contents('/var/log/nginx/blackjack.log', "card count = $card_count" . PHP_EOL, FILE_APPEND);
                $dbl_text = '';
                if ($a['ddown'] === 'yes') {
                ...
                if ($a['points'] != 21) {
                    ...
                    //file_put_contents('/var/log/nginx/blackjack.log', $doubleddown . PHP_EOL, FILE_APPEND);
                    //file_put_contents('/var/log/nginx/blackjack.log', mksize($blackjack['mb'], 0) . PHP_EOL, FILE_APPEND);
                ...
                if ($INSTALLER09['autoshout_on'] == 1 || $INSTALLER09['irc_autoshout_on'] == 1) {
                    ...
                    $message = "[color=#$classColor][b]{$CURUSER['username']}[/b][/color] has just played [color=#$opponent][b]{$a['username']}[/b][/color] $outcome ($points to {$a['points']}) $link.";
                    // crap
                    for ($i = 0; $i < $aces; ++$i) {
                        $points += ($points < 11 && $aces - $i == 1 ? 11 : 1);
                    }
                    ...
                    //file_put_contents('/var/log/nginx/blackjack.log', $message . PHP_EOL, FILE_APPEND);
                    autoshout($message);
                }
                ...
                $HTMLOUT .= "
						...
						<p>
							<a href='blackjack.php?id=$id' class='btn-clean'>{$lang['bj_play_again']}</a>
							<a href='games.php' class='btn-clean'>Games</a>
						</p>";
            } else {
                ...
                if ($INSTALLER09['autoshout_on'] == 1 || $INSTALLER09['irc_autoshout_on'] == 1) {
                    $classColor = get_user_class_color($CURUSER['class']);
                    //$message = "[color=#$classColor][b]{$CURUSER['username']}[/b][/color] has just played $link. :whistle: [color=gray]" . mt_rand(10, 30) . "[/color]";
                    $message = "[color=#$classColor][b]{$CURUSER['username']}[/b][/color] has just played $link.";
                    //file_put_contents('/var/log/nginx/blackjack.log', $message . PHP_EOL, FILE_APPEND);
                    autoshout($message);
                }
                $HTMLOUT .= "
...
//// crap
            ...
            if (!$ddown) {
                $HTMLOUT .= "
						<form method='post' action='".$_SERVER['PHP_SELF']."?id=$id'>
							<input type='hidden' name='game' value='hit' readonly='readonly' />
							<input class='btn-clean' type='submit' value='{$lang['bj_hitme']}' />
						</form>";
            }
            if ($points >= 17 && $dealer || $points >= 10 && !$dealer) {
                $HTMLOUT .= "
        ...
        //{$user_warning}";
        if ($waitarr['c'] > 0) {
            $r = sql_query("SELECT b.*, u.username, u.uploaded, u.downloaded, u.bjwins, u.bjlosses, u.id, u.class, u.donor, u.warned, u.leechwarn, u.enabled, u.chatpost, u.pirate, u.king FROM {$blackjack['version']} AS b LEFT JOIN users AS u ON u.id=b.userid WHERE b.status='waiting' AND b.userid != ".sqlesc($CURUSER['id']).' ORDER BY b.date ASC LIMIT 1') or sqlerr(__FILE__, __LINE__);
            $a = mysqli_fetch_assoc($r);
            $points_text = htmlsafechars($a['points'])." {$lang['bj_points2']}";
            $card_count = count(explode(' ', $a['cards']));
            //file_put_contents('/var/log/nginx/blackjack.log', "card count = $card_count" . PHP_EOL, FILE_APPEND);
            ...
            if ($INSTALLER09['autoshout_on'] == 1 || $INSTALLER09['irc_autoshout_on'] == 1) {
                $classColor = get_user_class_color($CURUSER['class']);
                $opponent = get_user_class_color($a['class']);
                $message = "[color=#$classColor][b]{$CURUSER['username']}[/b][/color] has just played [color=#$opponent][b]{$a['username']}[/b][/color] $outcome ({$playerarr['points']} to {$a['points']}) $link.";
                //file_put_contents('/var/log/nginx/blackjack.log', $message . PHP_EOL, FILE_APPEND);
                autoshout($message);
            }
...
//crap
	...
	<tr><td colspan='2'>
	<table class='table table-bordered text-center'>
	<tr><td><img src='{$INSTALLER09['pic_base_url']}cards/tp.bmp' width='71' height='96' border='0' alt='' class='tooltipper padded' /><img src='{$INSTALLER09['pic_base_url']}cards/vp.bmp' width='71' height='96' border='0' alt='' class='tooltipper padded' /></td></tr>
	$doubled
	<tr><td align='left'>
	<p>{$lang['bj_you_most_collect_21']}</p>
	<p>{$lang['bj_note']} {$game_str}</p>
	<p>You can lose 1.5 times your bet if you lose to a Natural Blackjack and a winning Natural Blackjack pays out at 1.5 times the bet.<br>The first player is considered the player and the second is considered the dealer. Only the player can double down on 9, 10, 11 and receive 1 card only. The dealer can see the upcards of the player but must draw a card for anything less than 17. No one wins a tie.</p></td></tr>
	<tr><td>
	<form method='post' action='".$_SERVER['PHP_SELF']."?id=$id'><input type='hidden' name='game' value='hit' readonly='readonly' /><input type='hidden' name='start_' value='yes' readonly='readonly' /><input class='btn-clean' type='submit' value='Start!' /></form>
	</td></tr></table>
	</td></tr></table>
	<br><br>
	<table class='table table-bordered text-center'>
		<tr>
			<td colspan='2'>
				<table class='table table-bordered text-center'>
					...
					<tr><td align='left'>{$lang['bj_wins']}</td><td>".human_filesize($tot_wins)."</td></tr>
					<tr><td align='left'>{$lang['bj_losses']}</td><td>".human_filesize($tot_losses)."</td></tr>
					<tr><td align='left'>{$lang['bj_win']} {$lang['bj_percentage']}</td><td>".htmlsafechars($win_perc)."</td></tr>
					<tr><td align='left'>+/-</td><td>".human_filesize($plus_minus).'</td></tr>
				</table>
			</td>
		</tr>
	</table>';
    ...
    // site stats
    $query = "SELECT username, bjwins * 1024 * 1024 * 1024 AS wins, bjlosses * 1024 * 1024 * 1024 AS losses, (bjwins - bjlosses) * 1024 * 1024 * 1024 AS sum,
					 id, class, donor, warned, leechwarn, enabled, chatpost, pirate, king
				FROM users
    ...
    if (count($bjgames) > 0) {
        ...
        foreach ($bjgames as $bjgame) {
            ...
            $HTMLOUT .= "
										...
										<tr>
											<td class='text-center' style='width:50%;'>
												".format_username($p1).': '.$bjgame['player1_points']."
											</td>
											<td class='text-center' style='width:50%;'>
												".format_username($p2).': '.${'points_'.$g}.'
											</td>
										</tr>
									</table>
								</td>
    ...
    $debugout = "
		<fieldset id='debug' class='header'>
			...
			<div id='table' class='table-responsive'>
				<table id='last10List' class='table table-hover table-condensed table-striped compact' width='100%'>
					{$debugout}
				</table>
			</div>
		</fieldset>";
