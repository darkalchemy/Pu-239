...
if ($id = $session->get('userID') && $session->validateToken($_POST['csrf_token'])) {
    ...
    if (!empty($seed['conn'])) {
        switch ($seed['conn']) {
            case 1:
                $connectable = "<img src='{$site_config['pic_baseurl']}notcon.png' alt='{$lang['gl_not_connectable']}' title='{$lang['gl_not_connectable']}' />";
                break;

            case 2:
                $connectable = "<img src='{$site_config['pic_baseurl']}yescon.png' alt='{$lang['gl_connectable']}' title='{$lang['gl_connectable']}' />";
                break;
