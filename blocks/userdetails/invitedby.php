...
if (mysqli_num_rows($rez_invited) < 1) {
    ...
    while ($arr_invited = mysqli_fetch_assoc($rez_invited)) {
        $inviteted_by_this_member .= '<tr><td>'.('pending' == $arr_invited['status'] ? htmlsafechars($arr_invited['username']) : format_username($arr_invited).'<br> '.($CURUSER['class'] < UC_STAFF ? '' : $arr_invited['ip'])).'</td>
        ...
        <td>'.('confirmed' == $arr_invited['status'] ? '<span style="color: green;">'.$lang['userdetails_confirmed'].'</span></td></tr>' : '<span style="color: red;">'.$lang['userdetails_pending'].'</span></td></tr>');
    }
    $inviteted_by_this_member .= '</table>';
}
$the_flip_box_5 = '[ <a name="invites"></a><a class="altlink" href="#invites" onclick="javascript:flipBox(\'5\')" name="b_5" title="'.$lang['userdetails_open_close_inv'].'">'.$lang['userdetails_inv_view'].'<img onclick="javascript:flipBox(\'5\')" src="'.$site_config['pic_baseurl'].'panel_on.gif" name="b_5" style="vertical-align:middle;"  width="8" height="8" alt="'.$lang['userdetails_open_close_inv1'].'" title="'.$lang['userdetails_open_close_inv1'].'" /></a> ] [ <a class="altlink" href="'.$site_config['baseurl'].'/staffpanel.php?tool=invite_tree&amp;action=invite_tree&amp;id='.(int) $user['id'].'" title="'.$lang['userdetails_inv_click'].'">'.$lang['userdetails_inv_viewt'].'</a> ]';
