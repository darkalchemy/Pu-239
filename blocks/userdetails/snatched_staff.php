...
if ($CURUSER['class'] >= UC_STAFF) {
    if (isset($_GET['snatched_table'])) {
        ...
        $HTMLOUT .= "<table border='1' cellspacing='0' cellpadding='5'><tr><td class='colhead'>{$lang['userdetails_s_cat']}</td><td class='colhead'>{$lang['userdetails_s_torr']}</td>" . "<td class='colhead'>{$lang['userdetails_s_sl']}</td><td class='colhead'>{$lang['userdetails_s_up']}" . ($site_config['ratio_free'] ? '' : "{$lang['userdetails_s_down']}") . "</td><td class='colhead'>{$lang['userdetails_s_tsize']}</td>" . "<td class='colhead'>{$lang['userdetails_ratio']}</td><td class='colhead'>{$lang['userdetails_client']}</td></tr>";
        while ($arr = mysqli_fetch_assoc($res)) {
            ...
            if (XBT_TRACKER === false) {
                $HTMLOUT .= "<tr><td>" . ($arr['owner'] == $id ? "<b><font color='orange'>{$lang['userdetails_s_towner']}</font></b><br>" : '' . ($arr['complete_date'] != '0' ? "<b><font color='lightgreen'>{$lang['userdetails_s_fin']}</font></b><br>" : "<b><font color='red'>{$lang['userdetails_s_nofin']}</font></b><br>") . '') . "<img src='{$site_config['pic_base_url']}caticons/" . get_categorie_icons() . "/" . htmlsafechars($arr['image']) . "' alt='" . htmlsafechars($arr['name']) . "' title='" . htmlsafechars($arr['name']) . "' /></td>" . "
