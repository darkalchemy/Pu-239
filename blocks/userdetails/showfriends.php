...
if (count($users_friends) > 0) {
    ...
    if ($users_friends) {
        foreach ($users_friends as $a) {
            $avatar = (($user['opt1'] & user_options::AVATARS) ? ($a['avatar'] == '' ? '<img src="' . $INSTALLER09['pic_base_url'] . 'forumicons/default_avatar.gif"  width="40" alt="default avatar" />' : '<img src="' . htmlsafechars($a['avatar']) . '" alt="avatar"  width="40" />') : '');
