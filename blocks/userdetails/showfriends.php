...
if (!empty($users_friends) && count($users_friends) > 0) {
    ...
    if ($users_friends) {
        foreach ($users_friends as $a) {
            $avatar           = (($user['opt1'] & user_options::AVATARS) ? ($a['avatar'] === '' ? '<img src="' . $site_config['pic_baseurl'] . 'forumicons/default_avatar.gif"  width="40" alt="default avatar" />' : '<img src="' . url_proxy($a['avatar'], true, 40, 'auto') . '" alt="avatar"  width="40" />') : '');
