...
if ($users_friends === false || is_null($users_friends)) {
    $fr = sql_query('SELECT f.friendid AS uid, f.userid AS userid, u.last_access, u.id, u.ip, u.avatar, u.offensive_avatar, u.username, u.class, u.donor, u.title, u.warned, u.enabled, u.chatpost, u.leechwarn, u.pirate, u.king, u.downloaded, u.uploaded, u.perms FROM friends AS f LEFT JOIN users AS u ON f.friendid = u.id WHERE userid = ' . sqlesc($id) . ' ORDER BY username ASC LIMIT 100') or sqlerr(__FILE__, __LINE__);
...
if (!empty($users_friends) && count($users_friends) > 0) {
    ...
    if ($users_friends) {
        foreach ($users_friends as $a) {
            ...
            $user_friends .= "<tr><td class='has-text-centered w-25 mw-150'>" . $avatar . '</td><td>' . format_username($user_stuff['id']) . '<br>' . ($CURUSER['class'] >= UC_STAFF ? '' . htmlsafechars($a['ip']) . '' : '') . "</td><td  style='padding: 1px'>" . mksize($a['uploaded']) . '</td>' . ($site_config['ratio_free'] ? '' : "<td  style='padding: 1px'>" . mksize($a['downloaded']) . '</td>') . "<td  style='padding: 1px'>" . member_ratio($a['uploaded'], $site_config['ratio_free'] ? '0' : $a['downloaded']) . "</td><td  style='padding: 1px'>" . $status . "</td></tr>\n";
        }
        $user_friends .= '</table>';
