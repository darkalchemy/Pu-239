...
if (count($users_friends) > 0) {
    ...
    if ($users_friends) {
        foreach ($users_friends as $a) {
            ...
            $user_friends .= "<tr><td class='one' style='padding: 0px; border: none' width='40px'>" . $avatar . "</td><td class='one'>" . format_username($user_stuff['id']) . '<br>' . ($CURUSER['class'] >= UC_STAFF ? '' . htmlsafechars($a['ip']) . '' : '') . "</td><td class='one' style='padding: 1px'>" . mksize($a['uploaded']) . '</td>' . ($site_config['ratio_free'] ? '' : "<td class='one' style='padding: 1px'>" . mksize($a['downloaded']) . '</td>') . "<td class='one' style='padding: 1px'>" . member_ratio($a['uploaded'], $site_config['ratio_free'] ? '0' : $a['downloaded']) . "</td><td class='one' style='padding: 1px'>" . $status . "</td></tr>\n";
        }
        $user_friends .= '</table>';
        $HTMLOUT .= "<tr><td class='rowhead' width='1%'>{$lang['userdetails_friends']}</td><td width='99%'><a href=\"javascript: klappe_news('a6')\"><img border='0' src='{$site_config['pic_base_url']}plus.png' id='pica6" . (int)$a['uid'] . "' alt='{$lang['userdetails_hide_show']}' title='{$lang['userdetails_hide_show']}' /></a><div id='ka6' style='display: none;'><br>$user_friends</div></td></tr>";
    } else {
